...
fn codec_derive_reports_unsupported_generated_helper_directions() {
    let source = SourceFile::new(
        "main.veln",
        concat!(
            "schema UnsupportedRecordHeader\n",
            "  format binary\n",
            "\n",
            "  control: String\n",
            "end\n",
            "\n",
            "codec UnsupportedDecode for UnsupportedRecordHeader decode\n",
            "  derive decode\n",
            "end\n",
            "\n",
            "codec UnsupportedEncode for UnsupportedRecordHeader encode\n",
            "  derive encode\n",
            "end\n",
        ),
    );
    ...
    assert!(
        diagnostics.iter().any(|diagnostic| {
            diagnostic.id == "schema.mapping_expression_unsupported"
                && diagnostic.message
                    == "schema mapping expression `{ value: kind } < length` is not supported"
        }),
        "{diagnostics:#?}"
    );
    assert!(
