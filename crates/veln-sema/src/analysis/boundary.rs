...
fn format_neutral_schema_helper_diagnostic(schema: &SchemaDecl, field: &SchemaField) -> Diagnostic {
    ...
    let boundary_message = format!(
        "Generated format-neutral decode helpers for schema `{schema_name}` accept only scalar fields, top-level List<Int>, List<Bool>, List<Float>, or List<String> fields, top-level Dict<String, Int>, Dict<String, Bool>, Dict<String, Float>, or Dict<String, String> fields, supported top-level Option fields, top-level Result<scalar, scalar> fields, and nested record-shaped fields with scalar, List<scalar>, Option<scalar>, Option<List<scalar>>, Option<Dict<String, scalar>>, Dict<String, scalar>, or Result<scalar, scalar> fields."
    );
    let mut diagnostic = Diagnostic::new(
