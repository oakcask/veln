...
fn declaration_facts(module: &SurfaceModule, base: Option<&TypeEnvironment>) -> DeclarationFacts {
    ...
    let published_adts = if base.is_none() {
        Some(
            crate::source_less_lookup::published_builtin_adt_registry()
                .expect("source-less lookup registries are valid"),
        )
    } else {
        None
    };
    let adt_base = base.map(|base| &base.adts).or(published_adts.as_ref());
    let adts = AdtRegistry::from_module_with_base(module, adt_base);
