...
fn run_case_with_guard_and_after_invocation(
    ...
    for run_index in 0..manifest.invocation.repeat {
        ...
        collect_run_failure(&mut run_failures, || {
            ...
            manifest
                .expectations
                .assert_matches(&context, &output, Some(&project.root));
            manifest
                .expectations
    ...
    fn validate(&self, path: &Path, index: usize) {
        ...
        if self.operation_count != 1 {
            manifest_error(
                ...
                0,
                format!(
                    "mcp_assert {index} needs exactly one of `equals`, `array_len`, `workspace_file_uri`, or `missing = true`"
                ),
            );
        }
    }
            ...
            ManifestStatement::Assignment { key, line, value } => {
                ...
                match section {
                    ...
                    Section::McpAssert(index)
                        if matches!(
                            *key,
                            "equals" | "array_len" | "workspace_file_uri" | "missing"
                        ) =>
                    {
...
impl<'a> ManifestParser<'a> {
    ...
    fn parse_mcp_assert_key(
        ...
        match key {
            ...
            "workspace_file_uri" => {
                assertion.operation_count += 1;
                assertion.operation = Some(McpAssertionOperation::WorkspaceFileUri(parse_string(
                    self.path, value,
                )));
            }
            "missing" => {
...
#[test]
fn decoded_mcp_jsonl_assertions_cover_ids_pointers_equality_arrays_and_missing() {
    let stdout = concat!(
        "{\"jsonrpc\":\"2.0\",\"id\":\"alpha\",\"result\":{\"escaped\":{\"a/b\":{\"m~n\":\"ok\"}},\"object\":{\"b\":2,\"a\":1},\"items\":[\"first\",\"second\"]}}\n",
        "{\"jsonrpc\":\"2.0\",\"id\":3,\"result\":{\"value\":true}}\n",
...
"#,
    ...
    let messages = decode_mcp_stdout(stdout).expect("JSONL stream should decode");
    for assertion in &assertions {
        evaluate_mcp_assertion(&messages, assertion, None).expect("assertion should pass");
    }
...
"#,
    ...
    .remove(0);
    let error = evaluate_mcp_assertion(&messages, &reversed, None)
        .expect_err("reversed array order should fail");
    assert!(error.contains("value mismatch"), "{error}");
}

#[test]
fn decoded_mcp_jsonl_rejects_bad_streams_and_response_selection_failures() {
    ...
    .expect("duplicates are assertion-local failures");
    let duplicate = parsed_mcp_assertions(
        "command = [\"mcp\"]\nexit = 0\n[[mcp_assert]]\nid = 1\npath = \"/result\"\nequals = null\n",
    )
    .remove(0);
    let error = evaluate_mcp_assertion(&messages, &duplicate, None)
        .expect_err("duplicate selected IDs should fail");
    assert!(error.contains("matched 2 message"), "{error}");

    let missing = parsed_mcp_assertions(
        "command = [\"mcp\"]\nexit = 0\n[[mcp_assert]]\nid = 7\npath = \"/result\"\nmissing = true\n",
    )
    .remove(0);
    let error = evaluate_mcp_assertion(&messages, &missing, None)
        .expect_err("missing selected ID should fail");
    assert!(error.contains("matched 0 message"), "{error}");
}

#[test]
fn decoded_mcp_workspace_file_uri_assertion_checks_canonical_safe_relative_files() {
    let root = test_temp_root("mcp-workspace-uri");
    fs::write(root.join("main.veln"), "fn main() -> Int\n  1\nend\n")
        .expect("workspace file should be written");
    let expected = workspace_file_uri(&root, "main.veln").expect("URI should be constructed");
    let stdout = format!(
        "{{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{{\"uri\":{}}}}}\n",
        JsonValue::String(expected).to_compact_string()
    );
...
"#,
    ...
    .remove(0);
    let messages = decode_mcp_stdout(&stdout).expect("JSONL stream should decode");
    evaluate_mcp_assertion(&messages, &assertion, Some(&root)).expect("URI should match");
    ...
    for relative in [
        ...
        assert!(error.contains("workspace_file_uri path"), "{error}");
    }
    let error = workspace_file_uri(&root, "missing.veln").expect_err("file should be required");
    assert!(error.contains("not readable"), "{error}");
    #[cfg(unix)]
    {
        std::os::unix::fs::symlink(root.join("main.veln"), root.join("linked.veln"))
            .expect("symlink should be created");
        let error =
            workspace_file_uri(&root, "linked.veln").expect_err("symlink should be rejected");
...
"#,
    ...
    let panic = std::panic::catch_unwind(|| {
        manifest
            .expectations
            .assert_matches(&context, &output, None)
    })
    ...
    let panic = std::panic::catch_unwind(|| {
        manifest
            .expectations
            .assert_matches(&context, &transport_output, None)
    })
...
"#,
    ...
    for (index, output) in outputs.iter().enumerate() {
        ...
        collect_run_failure(&mut failures, || {
            manifest.expectations.assert_matches(&context, output, None)
        });
    }
...
fn evaluate_mcp_assertion(
    ...
    let [selected] = matches.as_slice() else {
        return Err(format!(
            "selected response matched {} message(s), expected exactly one",
            matches.len()
        ));
    };
            ...
            .expect("validated MCP assertion operation")
        {
            ...
            McpAssertionOperation::WorkspaceFileUri(relative) => {
                ...
                let root = project_root.ok_or_else(|| {
                    "workspace_file_uri requires a copied project root".to_string()
                })?;
                let expected = workspace_file_uri(root, relative)?;
                if actual == expected {
                    Ok(())
                } else {
                    Err(format!(
...
fn workspace_file_uri(project_root: &Path, relative: &str) -> Result<String, String> {
    if relative.is_empty() {
        return Err("workspace_file_uri path must not be empty".to_string());
    }
    if relative.contains('\\') {
        return Err(format!(
            "workspace_file_uri path {relative:?} must use forward slashes"
        ));
    }
    let relative_path = Path::new(relative);
    if relative_path.is_absolute() {
        return Err(format!(
            "workspace_file_uri path {relative:?} must be workspace-relative"
        ));
    }
    for segment in relative.split('/') {
        if segment.is_empty() || segment == "." || segment == ".." {
            return Err(format!(
                "workspace_file_uri path {relative:?} has invalid segment {segment:?}"
            ));
        }
    }
    let path = project_root.join(relative_path);
    let metadata = fs::symlink_metadata(&path).map_err(|error| {
        format!("workspace_file_uri path {relative:?} is not readable: {error}")
    })?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(format!(
            "workspace_file_uri path {relative:?} must name an existing regular file"
        ));
    }
    let canonical = fs::canonicalize(&path).map_err(|error| {
        format!("workspace_file_uri path {relative:?} cannot be canonicalized: {error}")
    })?;
...
impl PartialEq for JsonValue {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::Object(left), Self::Object(right)) => json_object_entries_equal(left, right),
            _ => false,
        }
    }
}

fn json_object_entries_equal(left: &[(String, JsonValue)], right: &[(String, JsonValue)]) -> bool {
    ...
    for (left_key, left_value) in left {
        let Some(index) = right
            ...
            .position(|(index, (right_key, right_value))| {
                !used[index] && left_key == right_key && left_value == right_value
            })
        else {
            return false;
        };
