...
- `retirement_projection_evidence_test.veln`: generated standard-package test
  source that executes row-addressed projection SHA checks and the aggregate
...
executes every row id with its observed projection SHA in bounded row groups,
compares it with a generated expectation function, then checks the aggregate
digest for the same retained row set through the manifest expectation
function.
