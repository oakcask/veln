...
- `run/socket-stream-adapter-routing-spawn10/`: the same stream-adapter task
  ...
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn11/`: the same stream-adapter task
  boundary passes an eleventh ordinary metadata value through
  `task::spawn_with11`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn12/`: the same stream-adapter task
  boundary passes a twelfth ordinary metadata value through
  `task::spawn_with12`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn13/`: the same stream-adapter task
  boundary passes a thirteenth ordinary metadata value through
  `task::spawn_with13`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn14/`: the same stream-adapter task
  boundary passes a fourteenth ordinary metadata value through
  `task::spawn_with14`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn15/`: the same stream-adapter task
  boundary passes a fifteenth ordinary metadata value through
  `task::spawn_with15`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn16/`: the same stream-adapter task
  boundary passes a sixteenth ordinary metadata value through
  `task::spawn_with16`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn17/`: the same stream-adapter task
  boundary passes a seventeenth ordinary metadata value through
  `task::spawn_with17`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn18/`: the same stream-adapter task
  boundary passes an eighteenth ordinary metadata value through
  `task::spawn_with18`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn19/`: the same stream-adapter task
  boundary passes a nineteenth ordinary metadata value through
  `task::spawn_with19`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn20/`: the same stream-adapter task
  boundary passes a twentieth ordinary metadata value through
  `task::spawn_with20`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn21/`: the same stream-adapter task
  boundary passes a twenty-first ordinary metadata value through
  `task::spawn_with21`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn22/`: the same stream-adapter task
  boundary passes a twenty-second ordinary metadata value through
  `task::spawn_with22`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn23/`: the same stream-adapter task
  boundary passes a twenty-third ordinary metadata value through
  `task::spawn_with23`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn24/`: the same stream-adapter task
  boundary passes a twenty-fourth ordinary metadata value through
  `task::spawn_with24`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn25/`: the same stream-adapter task
  boundary passes a twenty-fifth ordinary metadata value through
  `task::spawn_with25`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn26/`: the same stream-adapter task
  boundary passes a twenty-sixth ordinary metadata value through
  `task::spawn_with26`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn27/`: the same stream-adapter task
  boundary passes a twenty-seventh ordinary metadata value through
  `task::spawn_with27`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn28/`: the same stream-adapter task
  boundary passes a twenty-eighth ordinary metadata value through
  `task::spawn_with28`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn29/`: the same stream-adapter task
  boundary passes a twenty-ninth ordinary metadata value through
  `task::spawn_with29`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn30/`: the same stream-adapter task
  boundary passes a thirtieth ordinary metadata value through
  `task::spawn_with30`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn31/`: the same stream-adapter task
  boundary passes a thirty-first ordinary metadata value through
  `task::spawn_with31`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn32/`: the same stream-adapter task
  boundary passes a thirty-second ordinary metadata value through
  `task::spawn_with32`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn33/`: the same stream-adapter task
  boundary passes a thirty-third ordinary metadata value through
  `task::spawn_with33`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn34/`: the same stream-adapter task
  boundary passes a thirty-fourth ordinary metadata value through
  `task::spawn_with34`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn35/`: the same stream-adapter task
...
- `run/socket-stream-adapter-routing-spawn36/`: the same stream-adapter task
  ...
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn37/`: the same stream-adapter task
  boundary passes a thirty-seventh ordinary metadata value through
  `task::spawn_with37`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn38/`: the same stream-adapter task
  boundary passes a thirty-eighth ordinary metadata value through
  `task::spawn_with38`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn39/`: the same stream-adapter task
  boundary passes a thirty-ninth ordinary metadata value through
  `task::spawn_with39`, preserving the existing `concurrency` effect and
  socket-free handler shape.
- `run/socket-stream-adapter-routing-spawn40/`: the same stream-adapter task
  boundary passes a fortieth ordinary metadata value through
  `task::spawn_with40`, preserving the existing `concurrency` effect and
  socket-free handler shape.
...
- `run/channel-first-stream-routing-nine-route/`: adapter-owned source routes
  ...
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-ten-route/`: adapter-owned source routes
  ordinary `StreamInput` values through ten typed channel routes, selects all
  ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-eleven-route/`: adapter-owned source
  routes ordinary `StreamInput` values through eleven typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twelve-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twelve typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-thirteen-route/`: adapter-owned source
  routes ordinary `StreamInput` values through thirteen typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-fourteen-route/`: adapter-owned source
  routes ordinary `StreamInput` values through fourteen typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-fifteen-route/`: adapter-owned source
  routes ordinary `StreamInput` values through fifteen typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-sixteen-route/`: adapter-owned source
  routes ordinary `StreamInput` values through sixteen typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-seventeen-route/`: adapter-owned source
  routes ordinary `StreamInput` values through seventeen typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-eighteen-route/`: adapter-owned source
  routes ordinary `StreamInput` values through eighteen typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-nineteen-route/`: adapter-owned source
  routes ordinary `StreamInput` values through nineteen typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-one-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty-one typed channel
  routes, selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-two-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty-two typed channel
  routes, selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-three-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty-three typed channel
  routes, selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-four-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty-four typed channel
  routes, selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-five-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty-five typed channel
  routes, selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
...
- `run/channel-first-stream-routing-twenty-seven-route/`: adapter-owned source
  ...
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-eight-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty-eight typed channel
  routes, selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-twenty-nine-route/`: adapter-owned source
  routes ordinary `StreamInput` values through twenty-nine typed channel
  routes, selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
- `run/channel-first-stream-routing-thirty-route/`: adapter-owned source
  routes ordinary `StreamInput` values through thirty typed channel routes,
  selects all ready routes in receiver-list priority order with
  `channel::select_many_priority`, and then invokes the same pure stream
  handler shape with explicit per-stream state.
...
- `check/socket-stream-adapter-routing-spawn11-effects/`: the eleven-argument
  stream-task boundary has the same effect requirements when a handler
  receives one additional ordinary metadata value through `task::spawn_with11`.
- `check/socket-stream-adapter-routing-spawn12-effects/`: the twelve-argument
  stream-task boundary has the same effect requirements when a handler
  receives one additional ordinary metadata value through `task::spawn_with12`.
- `check/socket-stream-adapter-routing-spawn13-effects/`: the
  thirteen-argument stream-task boundary has the same effect requirements when
  a handler receives one additional ordinary metadata value through
  `task::spawn_with13`.
- `check/socket-stream-adapter-routing-spawn14-effects/`: the
  fourteen-argument stream-task boundary has the same effect requirements when
  a handler receives one additional ordinary metadata value through
  `task::spawn_with14`.
- `check/socket-stream-adapter-routing-spawn15-effects/`: the
  fifteen-argument stream-task boundary has the same effect requirements when
  a handler receives one additional ordinary metadata value through
  `task::spawn_with15`.
- `check/socket-stream-adapter-routing-spawn16-effects/`: the
  sixteen-argument stream-task boundary has the same effect requirements when
  a handler receives one additional ordinary metadata value through
  `task::spawn_with16`.
- `check/socket-stream-adapter-routing-spawn17-effects/`: the
  seventeen-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with17`.
- `check/socket-stream-adapter-routing-spawn18-effects/`: the
  eighteen-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with18`.
- `check/socket-stream-adapter-routing-spawn19-effects/`: the
  nineteen-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with19`.
- `check/socket-stream-adapter-routing-spawn20-effects/`: the
  twenty-argument stream-task boundary has the same effect requirements when a
  handler receives one additional ordinary metadata value through
  `task::spawn_with20`.
- `check/socket-stream-adapter-routing-spawn21-effects/`: the
  twenty-one-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with21`.
- `check/socket-stream-adapter-routing-spawn22-effects/`: the
  twenty-two-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with22`.
- `check/socket-stream-adapter-routing-spawn23-effects/`: the
  twenty-three-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with23`.
- `check/socket-stream-adapter-routing-spawn24-effects/`: the
  twenty-four-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with24`.
- `check/socket-stream-adapter-routing-spawn25-effects/`: the
  twenty-five-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with25`.
- `check/socket-stream-adapter-routing-spawn26-effects/`: the
  twenty-six-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with26`.
- `check/socket-stream-adapter-routing-spawn27-effects/`: the
  twenty-seven-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with27`.
- `check/socket-stream-adapter-routing-spawn28-effects/`: the
  twenty-eight-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with28`.
- `check/socket-stream-adapter-routing-spawn29-effects/`: the
  twenty-nine-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with29`.
- `check/socket-stream-adapter-routing-spawn30-effects/`: the thirty-argument
  stream-task boundary has the same effect requirements when a handler
  receives one additional ordinary metadata value through
  `task::spawn_with30`.
- `check/socket-stream-adapter-routing-spawn31-effects/`: the
  thirty-one-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with31`.
- `check/socket-stream-adapter-routing-spawn32-effects/`: the
  thirty-two-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with32`.
- `check/socket-stream-adapter-routing-spawn33-effects/`: the
  thirty-three-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with33`.
- `check/socket-stream-adapter-routing-spawn34-effects/`: the
  thirty-four-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with34`.
- `check/socket-stream-adapter-routing-spawn35-effects/`: the
...
- `check/socket-stream-adapter-routing-spawn36-effects/`: the
  ...
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with36`.
- `check/socket-stream-adapter-routing-spawn37-effects/`: the
  thirty-seven-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with37`.
- `check/socket-stream-adapter-routing-spawn38-effects/`: the
  thirty-eight-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with38`.
- `check/socket-stream-adapter-routing-spawn39-effects/`: the
  thirty-nine-argument stream-task boundary has the same effect requirements
  when a handler receives one additional ordinary metadata value through
  `task::spawn_with39`.
- `check/socket-stream-adapter-routing-spawn40-effects/`: the forty-argument
  stream-task boundary has the same effect requirements when a handler
  receives one additional ordinary metadata value through `task::spawn_with40`.
...
- `check/channel-first-stream-routing-nine-route-effects/`: nine-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-ten-route-effects/`: ten-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-eleven-route-effects/`: eleven-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-twelve-route-effects/`: twelve-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-thirteen-route-effects/`: thirteen-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-fourteen-route-effects/`: fourteen-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-fifteen-route-effects/`: fifteen-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-sixteen-route-effects/`: sixteen-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-seventeen-route-effects/`:
  seventeen-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-eighteen-route-effects/`:
  eighteen-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-nineteen-route-effects/`:
  nineteen-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-route-effects/`: twenty-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
- `check/channel-first-stream-routing-twenty-one-route-effects/`:
  twenty-one-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-two-route-effects/`:
  twenty-two-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-three-route-effects/`:
  twenty-three-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-four-route-effects/`:
  twenty-four-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-five-route-effects/`:
...
- `check/channel-first-stream-routing-twenty-six-route-effects/`:
  ...
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-seven-route-effects/`:
  twenty-seven-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-eight-route-effects/`:
  twenty-eight-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-twenty-nine-route-effects/`:
  twenty-nine-route receiver-list channel-first stream routing keeps the same
  handler boundary: routing declares `concurrency`, and the handler remains
  effect-free.
- `check/channel-first-stream-routing-thirty-route-effects/`: thirty-route
  receiver-list channel-first stream routing keeps the same handler boundary:
  routing declares `concurrency`, and the handler remains effect-free.
