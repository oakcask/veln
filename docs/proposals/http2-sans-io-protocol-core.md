...
- These slices move frame-kind admission and part of payload application for
  SETTINGS, DATA, HEADERS, CONTINUATION, WINDOW_UPDATE, RST_STREAM, and GOAWAY
  receive shutdown, plus outbound GOAWAY, RST_STREAM, PRIORITY, DATA, and
  WINDOW_UPDATE send, including inbound DATA body accounting and outbound DATA
  body accounting. They do not yet move PUSH_PROMISE receive application,
  outbound HEADERS, outbound PUSH_PROMISE, wider content-length integration
  with outbound header emission, or wider output ordering out of the
  monolithic case.
...
- `examples/specification/run/http2-protocol-core/` still contains
  ...
  PUSH_PROMISE, outbound HEADERS, outbound PUSH_PROMISE, wider content-length
  integration with outbound header emission, complete stdout, output-chunk
  integration, and wider receive-dispatch ordering beyond
...
RST_STREAM, PRIORITY, DATA, and WINDOW_UPDATE send by moving PUSH_PROMISE
receive application, outbound HEADERS, outbound PUSH_PROMISE, wider
content-length integration with outbound header emission, and wider output
ordering behind
