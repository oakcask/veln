...
- These slices move frame-kind admission and part of payload application for
  ...
  PRIORITY, DATA, and WINDOW_UPDATE send, including inbound DATA body
  accounting, PUSH_PROMISE receive application, and outbound DATA body
  accounting. They do not yet move outbound HEADERS, outbound PUSH_PROMISE,
  wider content-length integration with outbound header emission, or wider
...
- `examples/specification/run/http2-protocol-core/` still contains
  ...
  outbound HEADERS, outbound PUSH_PROMISE, wider content-length integration
  with outbound header emission, complete stdout, output-chunk integration,
  and wider receive-dispatch ordering beyond
  ...
  PRIORITY, DATA, and WINDOW_UPDATE send boundaries.
...
GOAWAY, RST_STREAM, PRIORITY, DATA, and WINDOW_UPDATE send by moving outbound
HEADERS, outbound PUSH_PROMISE, wider content-length integration with outbound
header emission, and wider output ordering behind
