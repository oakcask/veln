...
| public HPACK request, response, and trailer header-list validation | standard-owned pure validation accepts completed public `http2::hpack::HeaderList` values, receives the active `SETTINGS_ENABLE_CONNECT_PROTOCOL` value for extended CONNECT, returns accepted content-length state, rejects duplicate pseudo-headers, pseudo-headers after regular headers, role-inappropriate pseudo-headers, missing request or response pseudo-headers, CONNECT and extended CONNECT shape errors, invalid status and informational END_STREAM responses, trailer pseudo-headers, uppercase or invalid ordinary names, connection-specific headers, invalid `te`, and invalid or mismatched `content-length` without mutating HPACK, stream, or frame input state | `header_list_validation_accepts_request_response_and_trailers`, `header_list_validation_reports_request_failures_with_context`, `header_list_validation_covers_connect_request_shapes`, `header_list_validation_covers_extended_connect_negotiation`, `header_list_validation_reports_response_and_trailer_failures`, `http2-core-header-list-validation`, `receive_frame_dispatch_preserves_state_on_header_failures`, and retained `http2-protocol-core` header integration | pure success, failed fact, selected header name, active-state label, accepted content-length projection, CONNECT and extended CONNECT negotiation moved to `core_test.veln`; focused external-package typechecking plus result and failure projection evidence moved to `http2-core-header-list-validation`; focused receive-frame projection moved to `http2-core-receive-frame-dispatch`; the monolithic case still owns decoded-name diagnostic projection, human/JSON diagnostics, complete stdout, output chunks, and wider ordering |
...
- `std::http2::core` now owns the immutable stream-level WINDOW_UPDATE receive
  ...
  state and failure projections.
- `std::http2::core` now owns the immutable connection-level WINDOW_UPDATE
  receive flow-control refill over `CoreConnectionState`: accepted connection
  WINDOW_UPDATE increments refill aggregate connection receive credit, while
  invalid increment and connection-window overflow expose focused context and
  preserve the input aggregate and preview. The adjacent `core_test.veln`
  checks the success and failure-state invariants, and the focused
  `http2-core-connection-window-update-flow-control` case records public state
  and failure projections.
- `std::http2::core` now owns a focused receive-frame dispatcher for migrated
  SETTINGS, DATA, HEADERS, CONTINUATION, WINDOW_UPDATE, RST_STREAM, and GOAWAY
  payload application. It decodes one complete frame, validates payload
  length, applies stream-frame admission, parses SETTINGS items and ACKs, DATA
  padding length, HEADERS prefixes, CONTINUATION fragments, WINDOW_UPDATE
  increments, RST_STREAM error codes, and GOAWAY last-stream-id plus
  error-code fields, and composes migrated peer SETTINGS state application,
  SETTINGS ACK bookkeeping, DATA flow-control, production HPACK header-block
  decoding, header-list validation, inbound DATA content-length body
  accounting, WINDOW_UPDATE flow-control, remote END_STREAM, RST_STREAM
  lifecycle, and connection shutdown transitions. The adjacent `core_test.veln`
  checks success, SETTINGS failure-source classification, HPACK, header-list,
  and content-length failure classification, accessor behavior, GOAWAY drain
  completion, GOAWAY boundary-raising rejection, and immutable failure-state
...
- `std::http2::core` now owns outbound GOAWAY send over
  ...
  boundary tightening, reject boundary increases without output or next state,
  and preserve empty output on encode failures. The adjacent `core_test.veln`
...
- `std::http2::core` now owns outbound DATA and WINDOW_UPDATE sends over
  `CoreConnectionState`: accepted DATA sends emit exact bytes, debit stream
  send credit, update body content-length observed counters, and apply local
  END_STREAM lifecycle; accepted WINDOW_UPDATE sends emit exact stream or
  connection bytes and refill receive credit immutably. Failures retain typed
  context with no output or next state for invalid streams, lifecycle
  rejection, send-window exhaustion, content-length shortfall, invalid
  increments, and receive-window overflow. The adjacent `core_test.veln`
  checks exact bytes, state updates, empty failure output, and immutable
  failure-state preservation; the focused `http2-core-outbound-data-flow` case
  records public result, failure-field, and output-chunk projections.
- `std::http2::core` now owns focused outbound HEADERS and PUSH_PROMISE sends
  over `CoreConnectionState` and the public production `std::http2::hpack`
  encoder. Accepted request HEADERS create immutable outbound stream state
  with content-length metadata and exact frame bytes. Accepted PUSH_PROMISE
  sends require a server endpoint, an associated open stream, and a
  server-initiated promised stream id, then reserve the promised stream
  locally with exact frame bytes. Header-list, endpoint, stream, GOAWAY, HPACK
  encode, and frame encode failures expose no output bytes or next state. The
  adjacent `core_test.veln` checks exact bytes, stream-state updates, empty
  failure output, and immutable input preservation; the focused
  `http2-core-outbound-headers` case records public result, failure-field, and
  output-chunk projections.
- These slices move frame-kind admission and part of payload application for
  ...
  DATA body accounting, and focused outbound header emission. They do not yet
  move wider content-length integration with outbound header emission or wider
  output ordering out of the monolithic case.
...
- Fixture HPACK state and compatibility routes remain in the aggregate case
  for residual integration paths; those residual routes still need to be
  converted to the public typed `std::http2::hpack` codec boundary before the
...
send by moving wider content-length integration with outbound header emission
and wider output ordering behind `std::http2::core`. Keep each transition
immutable and preserve
