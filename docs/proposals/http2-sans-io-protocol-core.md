...
- `std::http2::core` now owns focused outbound HEADERS and PUSH_PROMISE sends
  ...
  with content-length metadata and exact frame bytes. Accepted response
  HEADERS update existing stream content-length metadata without changing the
  observed body count, and accepted trailers preserve that metadata while
  closing the stream. Accepted PUSH_PROMISE sends require a server endpoint,
  an associated open stream, and a server-initiated promised stream id, then
  reserve the promised stream locally with exact frame bytes. Header-list,
  endpoint, stream, GOAWAY, HPACK encode, and frame encode failures expose no
  output bytes or next state. The adjacent `core_test.veln` checks exact
  bytes, stream-state updates, response/trailer content-length preservation,
