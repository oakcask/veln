...
`identifier-casing-source-path-boundary` examples. That current behavior
includes isolation from local import resolution, duplicate source-module
diagnostics, and dependency-cycle graph participation. Their completion record is
...
[Check JSON And Diagnostics](../specification/diagnostics-json.md), and
[Metrics JSON](../specification/metrics-json.md). Current behavior includes
local import, duplicate-module, and dependency-cycle graph isolation. This
proposal covers their unimplemented interactions with export, documentation,
backend, and deferred recovery consumers.
...
normal module registration. The invalid identity does not satisfy an import,
collide with a valid module, or contribute import edges or dependency cycles.
The remaining proposal separates deferred consumers by their observable
...
A target must not require an invalid source to reach an artifact stage that a
command specification blocks on source diagnostics. A proposal that changes
such a command to return a partial artifact must first define the new error,
output, selection, and policy-evaluation contract. A structurally invalid path
