...
| --- | --- | --- |
| Close the prerequisite client-platform set. | The platform-matrix proposal is complete and every plugin cell has a literal client-platform identity and compatibility field set. | [Closed Client-Platform Matrix](agent-language-services.md#closed-client-platform-matrix "matrix-ref:lifecycle-prerequisite-acceptance"), the completed matrix record, and checked exact key list, row count, field completeness, and digest form. |
