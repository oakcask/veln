...
- generated `byte_decode_<schema>` helper bindings for format-neutral schemas
  ...
  `Dict<String, Int>` or `Dict<String, String>`, a nested record shape made
  from scalar field types, or `Option<T>` where `T` is one of those scalar or
  nested record shapes, with declaration diagnostics for unsupported helper
  field types
...
- generated runtime decode bindings for binary schema fields outside the
  ...
  top-level `List<Int>` or `List<String>`, top-level `Dict<String, Int>` or
...
- Format-neutral schemas without a `format` clause whose fields are `Int`,
  ...
  top-level `Dict<String, Int>` or `Dict<String, String>`, nested record
  shapes made from scalar field types, or `Option<T>` where `T` is one of
  those scalar or nested record shapes, expose generated
