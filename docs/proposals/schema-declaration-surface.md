...
- generated `byte_decode_<schema>` helper bindings for format-neutral schemas
  ...
  `String`, a nested record shape made from those field types, or `Option<T>`
  where `T` is one of those scalar or nested record shapes, with declaration
...
- Format-neutral schemas without a `format` clause whose fields are `Int`,
  `Bool`, `Float`, `String`, nested record shapes made from those field types,
  or `Option<T>` where `T` is one of those scalar or nested record shapes
  expose generated `byte_decode_<schema>` helper bindings that accept and
  return the schema-local visible record through `Result<T, String>`.
...
- General schema decode can synthesize executable bindings for fields outside
  ...
  dispatch, and format-neutral scalar, supported `Option`, or nested
