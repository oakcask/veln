...
recognizes broad effect labels such as `net`, `time`, and `concurrency`, and
the first network runtime surface should build on those labels before adding
finer-grained network access modes.
...
The first network runtime surface should keep the existing coarse `net` effect
