...
Add the remaining high-level HTTP/2 client service boundary after the
...
boundaries to preserve callback effects through the proposed HTTP/2 client
reuse surface:
...
At the remaining client service boundary, a caller must not erase an
application effect by passing the callback through the service API. Handling
...
The remaining client surface owns connected-stream tasks and reuses a
...
The relative path is a planned directory below `examples/specification/`.
The remaining effect-row work is limited to the proposed HTTP/2 client service
API.
...
This proposal is complete when the client service boundary preserves callback
