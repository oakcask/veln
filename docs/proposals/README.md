...
- [Function Variadic Arguments](function-variadic-arguments.md): add
  source-level variadic parameters for ordinary functions and function values,
  with fixed-arity checks preserved before a homogeneous gathered tail.
...
- [Binary Schema Primitives And Dispatch](binary-schema-primitives-and-dispatch.md):
  ...
  closed-dispatch and extension-dispatch payload decode and encode support is
  implemented for the length-bounded forms when selected mappings cover every
  known case, resolve to one target record shape, and include a non-recursive
  base case.
...
- [HTTP/2 Sans-I/O Protocol Core](http2-sans-io-protocol-core.md): define the
  ...
  the HTTP/2 decode state, plus the explicit `0x3e` table-size update fixture
  carried through completed HEADERS and final CONTINUATION paths,
