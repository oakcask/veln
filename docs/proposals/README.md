...
- [Binary Schema Primitives And Dispatch](binary-schema-primitives-and-dispatch.md):
  ...
  decode, direct mapped-record encode, and direct ADT constructor mapped
  encode boundaries for supported schema-local fields,
...
- [HTTP/2 Sans-I/O Protocol Core](http2-sans-io-protocol-core.md): define the
  ...
  `RST_STREAM` reset send-intent, outbound HEADERS send-intent, and GOAWAY
  send-intent slices.
