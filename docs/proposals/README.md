...
- [Binary Schema Primitives And Dispatch](binary-schema-primitives-and-dispatch.md):
  ...
  closed-dispatch payload decode and encode support is implemented for the
  length-bounded `Dispatch(tag_field, length_field, ...)` form when selected
  mappings cover every case, resolve to one target record shape, and include a
