...
- `satisfy` suffix parsing, formatting, constraint exposure, missing candidate
  ...
  integer literal bounds, and inclusive integer literal bounds discharge
  adjacent strict integer literal bounds. Numeric literal bounds may include
  pure literal arithmetic subexpressions. Numeric literal equality
  ...
  excluded literal, including through equality aliases. Inclusive numeric
  bounds discharge their strict-or-equality `satisfy` disjunction. Statically
  satisfied repair candidates are retained even when they sort after the
  ordinary manual-review candidate bound.
  ...
  unrelated bindings.
  Valid function-entry requirements are also checked as one static implication
  against each substituted `satisfy` predicate, so safe repair discharge can
  use propositional consequences that only follow from multiple `require`
  clauses together.
...
- Contract predicates now parse through a dedicated first-slice predicate
  ...
  Negated partial case-split conjunctions whose disjunctive branches reject
