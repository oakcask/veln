...
payload decode slice is implemented when selected mappings cover every
dispatch case, all mappings resolve to one record shape, and at least one case
is non-recursive; recursive failures keep the outer dispatch field segment
before nested schema field segments. Imported private, missing, wrong-kind,
non-binary, forward, unbounded recursive, or otherwise ineligible payload
schemas use the existing `schema.dispatch_payload` diagnostic shape.
...
dispatch payload schemas outside the selected same-module closed-dispatch
decode slice, broader unsupported field layouts, and schema value mapping
beyond the implemented structural slices remain proposal work.
...
- recursive dispatch payload schemas outside the selected same-module
  closed-dispatch decode slice and dispatch payload schemas outside the
...
recursive dispatch payload schemas outside the selected same-module
closed-dispatch decode slice, dispatch payload schemas outside the generated
helper slice, and mapping beyond the implemented slices remain proposal work.
A `UInt31be` field
