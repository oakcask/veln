...
three-byte big-endian group where consecutive `UInt1` through `UInt7` fields
complete exactly one three-byte storage unit, preserving the same high-to-low
...
Generated schema helpers also decode and encode the narrow visible-only
five-byte big-endian group where consecutive `UInt1` through `UInt7` fields
complete exactly one five-byte storage unit, preserving the same high-to-low
declaration-order packing, ordinary `Int` fields, generated decode-step and
derived codec eligibility, truncation shape, and encode range-failure shape.
The completed five-byte visible-only group slice is archived under
`../reference/implemented-proposals/binary-schema-packed-visible-five-byte-groups.md`.
Generated schema helpers also decode and encode the narrow visible-only
six-byte big-endian group where consecutive `UInt1` through `UInt7` fields
complete exactly one six-byte storage unit, preserving the same high-to-low
declaration-order packing, ordinary `Int` fields, generated decode-step and
derived codec eligibility, truncation shape, and encode range-failure shape.
The completed six-byte visible-only group slice is archived under
`../reference/implemented-proposals/binary-schema-packed-visible-six-byte-groups.md`.
Generated schema helpers also decode and encode the narrow visible-only
seven-byte big-endian group where consecutive `UInt1` through `UInt7` fields
complete exactly one seven-byte storage unit, preserving the same high-to-low
declaration-order packing, ordinary `Int` fields, generated decode-step and
derived codec eligibility, truncation shape, and encode range-failure shape.
The completed seven-byte visible-only group slice is archived under
`../reference/implemented-proposals/binary-schema-packed-visible-seven-byte-groups.md`.
The
...
unknown tags preserve bounded raw payload bytes. Earlier same-module recursive
payload schemas and public imported recursive payload schemas named through
written `use` paths are also accepted for length-bounded parent dispatch
decode helpers without selected mappings when the payload schema already has
...
preservation, and nested helper diagnostics. The completed nested dispatch
...
The completed nested dispatch
`ByteView(left_length - right_length)` payload helper slice is archived under
`../reference/implemented-proposals/binary-schema-dispatch-byteview-subtract-payload-helpers.md`.
The completed nested dispatch
`ByteView(left_length * right_length)` payload helper slice is archived under
`../reference/implemented-proposals/binary-schema-dispatch-byteview-product-payload-helpers.md`.
The completed nested dispatch
`ByteView(left_length / right_length)` payload helper slice is archived under
`../reference/implemented-proposals/binary-schema-dispatch-byteview-quotient-payload-helpers.md`.
...
- general schema-declared length-prefixed payloads beyond the implemented
  ...
  slices, plus dispatch nested `ByteView(length_field)`,
  `ByteView(left_length + right_length)`,
  `ByteView(left_length - right_length)`,
  `ByteView(left_length * right_length)`, and
...
beyond the implemented nested generated-helper vocabulary, and mapping beyond
the implemented slices remain proposal work.
A `UInt31be` field
