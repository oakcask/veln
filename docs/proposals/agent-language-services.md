...
through MCP. The plugin does not bundle or download Veln. The client-platform
lifecycle set is exactly the closed matrix below.
...
resources available. Those server lifecycle checks apply only to the
[Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:agent-plugin-server-lifecycle").
...
These shared instructions are not a client-platform membership source.
...
add an installer only by first revising the [Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:agent-plugin-installer-boundary").

Each future compatibility record selected by the [Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:agent-plugin-compatibility-authority")
uses the exact compatibility fields and values in that section. It may replace
those values only after the client and validator artifacts exist. Shared skill content and
...
Every cell in the [Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:agent-plugin-native-validation")
...
resource template, lifecycle transition, and cell in the
...
declaration kinds, the LSP encodings, and the plugin compatibility cells named
by the [Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:conformance-capability-membership").
...
| Q21 plugin matrix | Pinned validators, generated-package freshness, every [Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:q21-plugin-matrix") boundary, native install, MCP smoke, Claude LSP smoke, and unknown-file isolation. |
...
proposal completes only when every declared cell in the
