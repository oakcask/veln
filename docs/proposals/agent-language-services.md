...
starts one server per active workspace root with that root as the process
working directory. A host mode is unsupported when it cannot set the working
directory or document and demonstrate inheritance from the workspace launch
directory. Initialization verifies that the client root resolves to the server
workspace base. It also exchanges the toolchain version, MCP contract,
...
add an installer after every row in the
[Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:plugin-installer-boundary")
exposes a sufficiently stable, non-interactive installation contract.
...
`compatibility.toml` records the compatibility-field identities listed above.
Later plugin work pins the host build, manifest-schema revision, validator
version, validator integrity digest, and required Veln, MCP, LSP,
language-service, and reference-schema contracts only after authoritative
client and validator artifacts establish those values. Widening a host range
requires adding and passing both boundary builds. Shared skill content and
`.mcp.json` are common authority. Each client manifest and Claude Code's
`.lsp.json` are authoritative only for that client. Client staging packages
omit files unknown to that client and are freshness-checked against the shared
inputs.
...
listing, resource reads, every tool, one domain failure, and shutdown. The
Claude Code row additionally performs the LSP initialize, initialized,
...
| Q22 gate totality | Injected missing requirement, duplicate evidence, missing [Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:q22-gate-totality") row, stale artifact, undeclared capability, malformed request class, and plugin mismatch. |
...
proposal completes only when every row in the
[Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:umbrella-completion")
passes and implemented behavior has been promoted to specification and
