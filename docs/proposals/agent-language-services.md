...
| Address empty, LF, CRLF, non-BMP, end-of-line, end-of-file, token-end, decimal and exponent integer spellings, and oversized positive positions. | Validity and half-open selection follow the scalar-coordinate contract, and unaddressable positive coordinates return `invalid_position`. | Implemented workspace-definition MCP coordinate table and stdio regressions for integral decimal and exponent spellings plus a value above `u64::MAX`; broader cross-adapter Q07 matrix remains planned. |
