...
direct-dependency schema-operation-reference and bounded direct-dependency
schema-alias operation-reference slices are
...
direct-dependency public schema operation and schema-alias operation leaves,
...
constructor, direct-dependency schema-operation, and direct-dependency
schema-alias operation-reference slices, paginated references, recovery and
casing-neutral reference navigation, package schema composition, other
dependency schema-alias references, standard-library schema and schema-alias
...
- definition lookup beyond the implemented workspace and package-backed symbol
  ...
  constructors, direct-dependency schema operations, and direct-dependency
  schema-alias operation references, including other dependency and
  standard-library public schema-alias references, plus paginated reference
  lookup;
- package schema composition, other dependency schema-alias reference lookup,
...
and saved workspace symbol `references` input and result schemas are checked
...
- schemas;
- public member aliases, including the implemented workspace schema-alias
  reference slice;
...
| Resolve references for every symbol in the closed v1 navigation matrix, including shadowing and same-spelled fields. | Only references with the selected symbol identity are returned in deterministic order. | Executable language-service and MCP tests cover the implemented workspace and package symbol classes, including workspace schema operations, composition targets, and public schema aliases plus direct-dependency schema operations and schema-alias operations. Package schema composition, standard-library schema, other package schema-alias classes, transitive-dependency, recovery, casing-neutral, unsupported package, and pagination rows remain planned. |
| Search references to a direct-dependency or standard-library function, type, constructor, public function alias, public type alias, or implemented direct-dependency schema or schema-alias operation leaf from one selected project. | Consumer uses are returned; other projects and package-internal uses are excluded, and the scope is explicit. | Executable language-service and MCP tests cover visible direct-dependency and standard-library functions, public aliases, types, constructors, and direct-dependency schema and schema-alias operation leaves without declaration or package-source inclusion. Focused MCP tests cover the schema-alias operation slice's selected-project and descendant boundaries, bounded retry without partial results or publication, and workspace-only results for path, vendor, mirror, and locally materialized git dependency sources. Package schema composition, standard-library schema, other package schema-alias classes, transitive-dependency, recovery, casing-neutral, and paginated package reference rows remain planned. |
...
boundary, and direct-dependency schema operation and schema-alias operation
reference search. It does not implement package schema composition,
standard-library schema, other package schema-alias classes,
...
access to private target functions. The saved-workspace symbol-reference
slice returns project-owned function, type, constructor, value-binding,
handler context-parameter, and handler operation clause-parameter reference
locations through MCP.
...
workspace public schema-alias, and direct-dependency schema operation and
schema-alias operation reference search are covered by executable
language-service and MCP tests. The direct-dependency schema-alias operation
tests cover selected-project scope, bounded stable-capture retry, and
workspace-only results across path, vendor, mirror, and locally materialized
git dependency sources.
Package reference search beyond the implemented function, function-alias, type,
type-alias, constructor, direct-dependency schema operation, and
direct-dependency schema-alias operation slices remains planned for schema
composition, standard-library schemas, other package schema-alias classes,
transitive dependencies, pagination, recovery, and casing-neutral references.
