...
`compatibility.toml` is the authoritative client matrix. The v1 matrix contains
exactly these client-platform rows:

| Client | Platform | Client-platform key |
| --- | --- | --- |
| `codex` | `x86_64-unknown-linux-gnu` | `codex/x86_64-unknown-linux-gnu` |
| `claude-code` | `x86_64-unknown-linux-gnu` | `claude-code/x86_64-unknown-linux-gnu` |

Each row pins these compatibility fields exactly once:

| Field | Planned value boundary |
| --- | --- |
| `client` | Literal client identifier from the closed client-platform row. |
| `platform` | Literal platform identifier from the closed client-platform row. |
| `host_build` | Checked client host build. |
| `manifest_schema_revision` | Checked client manifest-schema revision. |
| `validator_version` | Checked compatibility validator version. |
| `validator_digest` | Checked compatibility validator integrity digest. |
| `veln_contract` | Required Veln toolchain contract. |
| `mcp_contract` | Required MCP contract. |
| `lsp_contract` | Required LSP contract. |
| `language_service_contract` | Required language-service ABI contract. |
| `reference_schema_contract` | Required published-reference schema contract. |

The table closes membership only. It does not claim that any planned field
value has passed. Widening a host range requires adding and passing both
boundary builds in a separate proposal. Shared skill content and `.mcp.json`
are common authority. Each client manifest and Claude Code's `.lsp.json` are
...
resource template, lifecycle transition, and closed client-platform row.
...
| Q21 plugin matrix | Pinned validators, generated-package freshness, every closed client-platform row, native install, MCP smoke, Claude LSP smoke, and unknown-file isolation. |
| Q22 gate totality | Injected missing requirement, duplicate evidence, missing closed client-platform row, stale artifact, undeclared capability, malformed request class, and plugin mismatch. |
...
| Run the proposal completion gate. | Every requirement and evidence mapping, closed capability matrix, generated artifact, and closed client-platform row passes with no orphan. | Q22 conformance-manifest self-check and gate command. |
