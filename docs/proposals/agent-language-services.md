...
| Q05 stable capture | Implemented for `check_project` manifest, source, owned path-set changes, direct local dependency input changes and reuse, bounded retry, no partial publication, pre-refresh selection preservation, and selected-root symlink replacement. Concurrent navigation captures remain planned. |
