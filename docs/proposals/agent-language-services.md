...
The workspace-project inventory, saved project diagnostics, and bounded saved
...
current function, constructor, and handler-binding symbol set. It also exposes
bounded, cursor-paged `references` for that same saved workspace symbol set.
Broader navigation, documentation resources and search, snapshot resource
lifetime, conformance completion, and client plugin work in this proposal
remain planned.
...
| Q02 descendant ownership | Implemented for workspace navigation project inference and unselected descendant single-file isolation within the current symbol set. |
...
rows implemented by current slices point to current specification and
executable evidence. Rows that retain planned evidence do not imply that the
...
| Navigate below an unselected descendant manifest. | The outer project does not own the source; navigation reports single-file scope without outer-project references. | Implemented MCP definition and reference descendant-boundary isolation. |
| Add, remove, or rename a manifest. | Selection is unchanged until `refresh_workspace`; a successful refresh replaces it atomically. | Implemented `veln-mcp` refresh transitions and reference-cursor staleness tests. |
...
| Change a selected root identity, anonymous base identity, manifest, source, dependency input, or file set during capture. | The complete capture retries at most three times, then returns `snapshot_changed` without partial publication. | Implemented `veln-mcp` stable-capture retry tests for `check_project`, including anonymous base and selected-root symlink and regular-directory replacement, nested regular manifest marker boundaries, symlinked nested manifest marker exclusion, project-local source symlink exclusion, non-Linux fail-closed capture, and dependency snapshot changes; implemented workspace navigation capture coverage and reference capture failure. |
...
returns `file:` definitions and bounded reference pages for functions, type
constructors, handler context parameters, handler operation clause parameters,
and exact test-companion access to private target functions. Dependency and
standard-library locations and the proposal's additional symbol kinds remain
planned.
...
2. Extend the existing `veln mcp` server with resources, documentation tools,
   and navigation beyond the implemented workspace-only symbol inventory.
3. Add cross-adapter conformance cases and bounded documentation search.
