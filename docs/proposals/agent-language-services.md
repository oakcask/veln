...
A later proposal may add an installer after the client set in
[Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:agent-plugin-installer-boundary")
exposes a sufficiently stable, non-interactive installation contract.

Shared skill content and `.mcp.json` are common authority for each cell in
...
resource template, lifecycle transition, and cell in
...
declaration kinds, the LSP encodings, and the cells in
...
| Q21 plugin matrix | Pinned validators, generated-package freshness, every cell in [Closed Client-Platform Matrix](#closed-client-platform-matrix "matrix-ref:q21-plugin-matrix"), native install, MCP smoke, Claude LSP smoke, and unknown-file isolation. |
...
proposal completes only when every declared
