...
The codec-owned padding mismatch diagnostic slice is implemented for
`codec.padding_mismatch` direct `DecodeErrorWithReason(...)` failures and
`DecodeStep::Invalid(DecodeErrorWithReason(...))` failures. It carries field
path, expected padding length, actual padding length, and failure reason in
`details.byte_diagnostic`, keeps the human primary focused on the padding
mismatch at the byte offset, and is checked by
...
`../../examples/specification/run/codec-padding-mismatch-step-json/`, and
...
`DecodeStep::Invalid(DecodeErrorWithReason(...))` failures. It carries field
path, expected sequence, actual sequence, and failure reason in
`details.byte_diagnostic`, keeps the human primary focused on the sequence
mismatch at the byte offset, and is checked by
...
The codec-owned magic mismatch diagnostic slice is implemented for
`codec.magic_mismatch` direct `DecodeErrorWithReason(...)` failures and
`DecodeStep::Invalid(DecodeErrorWithReason(...))` failures. It carries field
path, expected magic, actual magic, and failure reason in
`details.byte_diagnostic`, keeps the human primary focused on the magic
mismatch at the byte offset, and is checked by
`../../examples/specification/run/codec-magic-mismatch-direct-json/`,
`../../examples/specification/run/codec-magic-mismatch-direct-human/`,
`../../examples/specification/run/codec-magic-mismatch-step-json/`, and
`../../examples/specification/run/codec-magic-mismatch-step-human/`.
The codec-owned unsupported feature diagnostic slice is implemented for
`codec.unsupported_feature` direct `DecodeErrorWithReason(...)` failures and
`DecodeStep::Invalid(DecodeErrorWithReason(...))` failures. It carries field
path, unsupported feature, and failure reason in `details.byte_diagnostic`,
keeps the human primary focused on the unsupported feature failure at the
byte offset, and is checked by
...
`../../examples/specification/run/codec-unsupported-feature-step-json/`, and
...
The completed codec-owned padding mismatch slice is archived under the
[implemented proposal record](../reference/implemented-proposals/codec-padding-mismatch-diagnostics.md).
...
The completed codec-owned magic mismatch slice is archived under the
