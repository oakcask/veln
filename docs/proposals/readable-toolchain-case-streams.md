...
The following questions change accepted manifests, invocation bytes,
comparison values, or observable failures. Resolve each question before the
implementation slice that depends on it. When a question is resolved, replace
it with the corresponding acceptance rows and planned evidence.

### Manifest Grammar

- **M1: TOML compatibility boundary.** Which TOML version and subset define
  multiline basic strings, multiline literal strings, and their delimiters?
  The decision must cover Unicode and control-character escapes, line-ending
  backslash folding, embedded one- and two-quote runs, terminal four- and
  five-quote runs, and whether single-line literal strings are valid array
  elements. Alternatively, the grammar can retain only the current manifest
  escape and element repertoire, but it must not call unsupported TOML forms
  compatible.
- **M2: Structural indentation.** Are spaces and tabs before content and a
  closing delimiter preserved as value bytes, as in TOML, or does the manifest
  grammar remove structural indentation? For example, the indented literal in
  a multiline `contains` array currently has more than one possible decoded
  value.
- **M3: Physical line endings.** Does a physical CRLF in a multiline manifest
  value decode as CRLF or normalize to LF? The answer must also cover the
  opening-newline rule and line-ending backslash folding so migrated invocation
  and expected bytes do not depend accidentally on the checkout platform.
- **M4: Array grammar by field type.** Are multiline arrays limited to fields
  whose existing type is a string array, or may a JSON-valued `equals` array
  also span lines? If JSON-valued arrays may span lines, do they retain JSON
  syntax or accept TOML comments, trailing commas, and multiline string
  elements?
- **M5: Multiline error locations.** For invalid escapes, quote runs, array
  elements, missing commas, and tokens after a closing delimiter, does the
  diagnostic report the offending physical line or the value's opening line?
  The answer must distinguish local errors from cross-line errors while
  retaining the specified opening-line location for an unterminated value.

### Case Files And Migration

- **F1: Portable reference grammar.** What platform-independent path grammar
  defines a case-file reference? The decision must cover separators, `.`
  components, repeated separators, drive-relative spellings, platform path
  prefixes, and other spellings that host `Path` implementations classify
  differently.
- **F2: Containment threat model.** Is the discovered fixture assumed to be a
  trusted static tree while a reference is validated and read, or must the
  no-link and containment guarantees survive concurrent path replacement?
  The decision must also define whether junctions or other reparse points are
  links and whether the discovered tree, copied tree, or both are checked.
- **F3: Snapshot lifecycle.** Are `stdin_file`, `equals_file`, and `$case_text`
  bytes read once and reused for every repeated invocation, or can any value be
  reread? If loading is eager, does an invalid referenced file fail a case
  before its skip rules are evaluated?
- **F4: Text byte policy.** Are a UTF-8 byte-order mark and CRLF preserved as
  content in text sidecars? How are they handled in structured JSON fixtures?
  The answer must state whether repository attributes fix exact-text sidecars
  to LF or whether exact checks intentionally observe checkout-specific bytes.
- **F5: Alternative cardinality.** What is the complete validation matrix for
  `stdin`, `stdin_file`, `stdin_jsonrpc_file`, `equals`, `equals_file`, and
  `missing` in every section that accepts them? In particular, the matrix must
  distinguish optional root stdin, exact string assertions, non-string JSON
  equality, missing-path assertions, file assertions, and raw stream sections.
- **G1: Discovery boundary.** Does migration and policy validation cover
  exactly the cases generated by the current build-time discovery algorithm,
  which stops descending after finding a `case.toml`, or every manifest below
  either root? The answer must also cover skipped cases, nested manifests, and
  manifests reached through links.
- **G2: Encoded-line-break predicate.** Which exact spellings and syntactic
  regions does the zero-escape gate reject? The decision must distinguish
  `\n`, `\\n`, literal-string text, Unicode or numeric encodings, embedded JSON
  strings, and comments, and must say whether the check is a raw-byte scan, a
  token-aware scan, or a decoded-value rule.
- **G3: Policy timing.** Does the zero-escape gate scan every in-scope manifest
  before parsing and skip evaluation, or does it run as each generated case is
  loaded? The failure ordering and whether all violations can be reported in
  one run must be part of the planned policy evidence.
- **G4: Migration equivalence evidence.** What authoritative inventory or
  before-and-after evidence proves that every migrated case retains its prior
  assertion boundary? The evidence must detect a weakened exact or fragment
  assertion while allowing an explicitly reviewed raw-to-decoded LSP
  conversion.

### Structured JSON-RPC And LSP Assertions

- **J1: Request validation boundary.** Does `stdin_jsonrpc_file` frame every
  JSON object, including intentionally protocol-invalid objects, or reject
  elements that are not valid JSON-RPC client requests or notifications before
  command execution? If it validates the protocol, define the required members
  and value kinds and route intentional invalid-frame cases to a raw input
  representation.
- **J2: JSON data model.** Do structured request fixtures and decoded messages
  accept the complete selected JSON standard or the current harness subset?
  The answer must cover fractional and exponent numbers, Unicode escapes and
  surrogate pairs, duplicate object members, and preservation or rejection of
  member order and duplicates during serialization.
- **J3: `$case_text` scope.** At which object and array depths is `$case_text`
  recognized, and is the top-level message-object requirement checked again
  after substitution? The decision must also say whether an object containing
  only a literal `$case_text` application field can be represented and confirm
  that inserted file text is never reparsed as JSON or as another directive.
- **J4: Request equivalence.** Must migration to `stdin_jsonrpc_file` preserve
  the exact framed body bytes, or only decoded message values, array order, and
  correct UTF-8 byte lengths? This resolves the current tension between the
  general same-invocation-bytes rule and serializer-independent LSP behavior.
- **J5: Identifier domain.** Which JSON-RPC identifier forms may
  `lsp_assert.id` select, and does selection use exact JSON type-and-value
  equality? The decision must address at least integer and string identifiers,
  `null`, and whether numerically equivalent spellings can compare equal.
- **J6: Message classification.** How does decoded stdout distinguish
  responses, notifications, and server-to-client requests? Define which class
  participates in an `id` selector, which class participates in a `method`
  selector, and whether an unsupported server request is ignored or fails
  decoding.
- **J7: Selector cardinality.** Is a duplicate response identifier an error
  across the complete decoded stream or only when an assertion selects it?
  May multiple assertions reuse one selector to inspect different paths? For
  notifications, does `occurrence` select one indexed match while permitting
  additional matches, or also impose a total-cardinality requirement?
- **J8: Frame decoder contract.** Which header spellings, separators, optional
  headers, duplicate or conflicting `Content-Length` fields, body encodings,
  and decoded JSON roots form a valid stdout frame? Decide whether semantic LSP
  assertions deliberately use a stricter, equal, or more tolerant framing
  contract than the LSP implementation under test.
- **J9: Partial output and process failure.** Must stdout be a complete frame
  sequence with no trailing bytes? Are all configured LSP, exit, stdout, and
  stderr assertions still enforced after a nonzero exit or a partial final
  frame, or does one failure short-circuit the others?
- **J10: Assertion path grammar.** What path grammar addresses the selected
  message root, dotted object keys, numeric object keys, and array indexes?
  Define which absent members, out-of-range indexes, and wrong-kind traversals
  satisfy `missing` and which fail as an invalid path.
