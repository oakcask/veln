...
- `run`: entry resolution, fixed and variadic entry argument conversion,
  ...
  frame-size exceeded, fixed payload length, and invalid PRIORITY dependency
  likewise use the same human runtime diagnostic rendering as the
  compatibility helpers, with the stable id, protocol facts, provenance, and
  bounded byte preview projected from the returned `RuntimeDiagnostic(...)`
