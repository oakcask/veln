...
`definition` declaration plus response-local definition, no-definition,
decimal and exponent integer coordinate spellings, invalid-position results,
non-integer decimal and negative-exponent coordinate schema rejection,
canonical workspace `file:` URI spelling, singleton content cardinality, array
indexing, result/error separation, and absent success-only fields over stdio.
