...
human-output evidence. The `identifier-casing-inferred-callable-recovery` and
`contract-result-binding-scope` cases check call-target recovery diagnostics,
including typed local recovery ambiguity and the boundary that prevents result
