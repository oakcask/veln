...
- source-backed pure helpers: `vec_len`, `vec_is_empty`, `vec_push`,
  ...
  `result_and_then`
- descriptor-only pure helpers: `vec_fold`, `string_split_once`,
...
using the existing prelude runtime operation: `dict_get`, `dict_insert`, and
`dict_remove` are source-backed descriptor entry points, and `dict_contains`
derives its result from `dict_get`. Private support functions such as
`vec_try_map_with_step` are ordinary support source and are not separate
prelude descriptors.
