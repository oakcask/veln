...
This page specifies and routes implemented JSON output for `veln test
--json`.
...
- Top-level fields: [Envelope](#envelope).
...
- Counts in `summary`: [Summary](#summary).
- Case records, doctests, runtime failures, runtime expectation mismatches,
  ...
  [Cases](#cases).
- Static gate behavior: [Static Gate](#static-gate).
...
attributes use `doctest.unknown_metadata`; empty `error=`, empty or
unsupported runtime expectation metadata, missing runtime expectation details,
missing `stream`, duplicate `stream` attributes on one output fence, and
unsupported output streams use `doctest.invalid_metadata`.
