...
continuation ordering failure, an accepted unknown extension frame after the
client preface gate that preserves flags, stream id, and bounded payload bytes
in an ordinary `UnknownFrame` value, an unknown frame rejected by active
continuation state, and an incoming frame whose payload length exceeds the
active receive maximum frame size, plus a DATA frame kind rejected for
connection-control state and idle-stream state. It also pins PING frames
