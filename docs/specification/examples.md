...
`../../examples/specification/run/codec-sequence-mismatch-step-human/`, and
`../../examples/specification/run/codec-sequence-mismatch-step-json/` cover the
same command-facing projection for `codec.sequence_mismatch`: human output
uses a focused sequence-mismatch diagnostic, and `run --json` attaches
`details.byte_diagnostic` with the codec-owned id, byte offset, field path,
...
covers the same derived codec decode and encode item boundary for a
visible-only packed four-byte group. It checks successful `Decoded`,
short-input `NeedMore(NeedBytes(...))`, budgeted encode resume to `Encoded`,
and helper-projected `Invalid(EncodeError(...))` outcomes through the codec
item.
The executable specification case
...
covers the same derived codec decode and encode item boundary for a
visible-only packed six-byte group.
The executable specification case
`../../examples/specification/run/derived-codec-packed-visible-seven-byte-boundary/`
covers the same derived codec decode and encode item boundary for a
visible-only packed seven-byte group. It checks successful `Decoded`,
short-input `NeedMore(NeedBytes(...))`, budgeted encode resume to `Encoded`,
and helper-projected `Invalid(EncodeError(...))` outcomes through the codec
item.
The executable specification case
...
covers the six-byte visible-only packed generated helper boundary. It checks
direct decode, generated decode-step, derived codec decode, direct encode,
and derived codec encode over one shared big-endian storage unit.
The executable specification case
`../../examples/specification/run/binary-schema-packed-visible-seven-byte-decode-encode/`
covers the seven-byte visible-only packed generated helper boundary. It
checks direct decode, generated decode-step, derived codec decode, direct
encode, and derived codec encode over one shared big-endian storage unit.
The executable specification case
...
The executable specification case
`../../examples/specification/run/derived-codec-packed-visible-four-byte-boundary/`
covers the derived codec encode boundary over a visible-only packed four-byte
group, including budgeted partial output, resumed `Encoded` output, and
helper-projected `Invalid(EncodeError(...))`.
The executable specification case
`../../examples/specification/run/derived-codec-packed-visible-five-byte-boundary/`
...
The executable specification case
`../../examples/specification/run/derived-codec-packed-visible-six-byte-boundary/`
covers the same derived codec encode boundary over a visible-only packed
six-byte group.
The executable specification case
`../../examples/specification/run/derived-codec-packed-visible-seven-byte-boundary/`
covers the same derived codec encode boundary over a visible-only packed
...
pin same-module, written imported, and unqualified public imported pure
...
pin the same imported converter inverse projection when the mapping expression
and inverse name are public function aliases imported into the schema module.
...
pin the same generated helper eligibility through a `derive encode` codec
boundary.
...
eligible recursive payload schema through the same generated helper path. The
recursive extension-dispatch case pins the same known-payload helper path
while unknown tags preserve bounded raw payload bytes. The imported recursive
decode case pins the same closed and extension dispatch behavior when the
recursive payload schema is public and named through a written `use` path. The
...
produced by the nested helper. The recursive failure cases pin that same path
prefix for a nested length-boundary failure, including the wrapper and
imported recursive paths. The
...
labels for every byte value. Multi-byte decoded non-visible byte strings use
deterministic `hpack-bytes-xx-...-xx` labels; the existing
`hpack-bytes-00-ff` label remains the spelling for decoded bytes
`0x00 0xff`, and the checked two-NUL decode produces `hpack-bytes-00-00`.
The fixture
...
are `0x04 0x85 0xff 0xc7 0xff 0xff 0xdd`. It keeps only the outbound
raw-string encoder's multi-byte non-visible value on the raw string encoding
failure path. The
...
Multi-byte non-visible Huffman strings now decode to `hpack-bytes-*` labels
instead of the focused non-visible failure path. The focused failures
...
decode as `hpack-bytes-*` fixture labels. The focused
...
stream credit as stable rejected no-output outcomes. Received
`SETTINGS_INITIAL_WINDOW_SIZE` changes apply their delta to existing tracked
open-stream send credit; a reduction can make that credit negative and block
the same DATA intent, a later stream-level peer `WINDOW_UPDATE` can restore
enough credit for it to emit bytes, and an increase raises existing send
credit by the same delta. Valid peer `WINDOW_UPDATE` frames can then refill
the separate outbound connection or stream send credit, allowing the same DATA
payload to emit bytes after the matching credit is restored. The checked case
also keeps local outbound `WINDOW_UPDATE` receive-credit intents from
refilling outbound DATA send credit.
...
`../../examples/specification/run/hpack-static-codec-boundary/`,
