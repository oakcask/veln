...
dispatch fields, reserved or fixed fields, and derived codec encode execution
are outside that encode helper slice.
...
`byte_encode_<schema>` helpers for the exact-width primitive encode slice, their
