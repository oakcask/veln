...
shape made from those field types. Unsupported format-neutral helper fields
are declaration diagnostics. When present, the single `format binary` clause
must appear before schema fields.
