...
`Dict<String, String>`, a nested record shape made from scalar or
`Option<scalar>` field types, or `Option<T>` where `T` is one of those scalar
or nested record shapes.
