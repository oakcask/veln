...
converted command-facing frame-size, header-list-size, invalid-frame-kind, and
priority self-dependency examples check this boundary for both
`http2.peer_limit.*` and `http2.protocol.*` failures.
...
even client stream ids, nonzero stream ids on connection-only frames, and
