...
values, and advances by the shared storage width for the pair. The inverse
...
same two-byte big-endian storage unit. That form decodes the visible value
from the high bits, validates the low reserved bits at the reserved field
path, omits the reserved field, and advances by the shared storage width.
...
visible `UIntN` field followed by a `ReservedBits(width, value)` suffix that
completes the same one-byte or two-byte big-endian storage unit is
