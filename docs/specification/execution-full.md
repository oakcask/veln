...
same two-byte, three-byte, four-byte, five-byte, six-byte, seven-byte, or eight-byte
big-endian storage unit.
...
returns `Err(RuntimeDiagnostic(id, message, RuntimeByteDiagnostic(...)))` and
...
The rendered `RuntimeDiagnostic(...)` is the result value projected by command
output.
...
`use` path or alias with one, two, three, four, or five arguments. Each argument must be
...
repeat-backed schemas, arithmetic-count and quotient-count repeat schemas,
quotient-sized
`ByteView(left_length / right_length)` payload fields, product-sized
`ByteView(left_length * right_length)` payload fields, additive
`ByteView(left_length + right_length)` payload fields, subtractive
...
payload schemas, repeat-backed schemas, arithmetic-count and quotient-count
repeat schemas, direct structural mappings, selected structural mappings, and
additive, subtractive, product-sized, and quotient-sized `ByteView` payload
fields already accepted by `byte_encode_<schema>`.
...
nested schema decoded record shape. Earlier same-module recursive payload
schemas and public imported recursive payload schemas named through written
`use` paths may also decode through a length-bounded parent without selected
mappings when the payload schema has bounded recursive helper support and the
parent includes at least one non-recursive primitive case; that boundary is
decode-only. `Known(value)` writes
...
the same two-byte, three-byte, four-byte, five-byte, six-byte, seven-byte, or eight-byte
big-endian storage unit.
...
`net::write_chunk_until` writes one immutable `ByteChunk` before the supplied
`Deadline` and returns `WriteCompleted`, or reports supplied or fixture write
deadline expiry as `WriteDeadlineExpired`,
`net::write_chunk_until_cancellable` writes one immutable `ByteChunk` before
the supplied `Deadline` and before cancellation and returns
`WriteCompleted`, reports supplied or fixture write deadline expiry as
...
to that stream, `net::write_chunk_until` writes response bytes before the
supplied deadline or returns a write outcome value for deadline expiry,
`net::write_chunk_until_cancellable` writes response bytes before the
