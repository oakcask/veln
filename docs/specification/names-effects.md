...
- Source ADT type and constructor names start with an ASCII uppercase letter.
  ...
  source-visible descriptor's module segments, spelling, and lookup key before
  publishing a registry. Qualified lookup keys are the exact module-name and
  ...
  `name.invalid_case`. Compiler adapter names that cannot participate in
  source lookup stay outside this gate. The focused `veln-sema`
  `standard_symbols` tests define the generated-table, injected-descriptor,
  duplicate-key, atomic-failure, checked-lookup, and lookup-isolation evidence
  for this boundary. Shared standard-environment initialization validates the
  registry before publishing reusable command or adapter state.
