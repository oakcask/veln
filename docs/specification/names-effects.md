...
- Source ADT type and constructor names start with an ASCII uppercase letter.
  ...
  `identifier-casing-companion-target-recovery-isolation-json` and
  `identifier-casing-companion-source-recovery-isolation-json` test cases
  define that quarantined invalid casing recovery records do not resolve uses
  between an exact `.test.veln` companion and its target source. The
