...
- Source-visible `StreamInput`, `DecodeStep<T>`, `DecodeReadiness`,
  ...
  Generated binary schema encode helpers for the exact-width and supported
  reserved-bit primitive slices accept schema-local visible `Int` record
  fields and return `Result<ByteChunk, EncodeError>`. HTTP/2 frame-header
  decoding, bounded payload frame decoding, and protocol diagnostic projection
  helpers are
