...
This page specifies implemented name resolution, effect checking, and
...
- Source ADT type and constructor names start with an ASCII uppercase letter.
  ...
  `rename.invalid_case` rejection. The same LSP case defines that bare nullary
  constructor expression and pattern uses from source-declared constructors are
  edited by constructor rename, that same-spelled effect and effect operation
  names are not selected or edited as type rename targets, that ambiguous bare
  imported type references have no selected symbol, and that qualified type
  references preserve their target identity.
...
- Namespaces, shadowing, duplicate checks, module ownership, external package
  ...
  [names-effects.md](names-effects.md#name-resolution).
- Declaration effect spelling, effect-row substitution, host effect labels,
  ...
  [names-effects.md](names-effects.md#effect-labels) and
  [names-effects.md](names-effects.md#concurrency-calls).
...
- Private `std::diagnostic` ownership, public prelude aliases, and the nested
  ...
  [Prelude Helpers](names-effects.md#prelude-helpers).
- Compiler-known calls:
  [stdio](names-effects.md#stdio-calls),
  [file-system](names-effects.md#file-system-calls),
  [network and time](names-effects.md#network-and-time-boundary-calls),
  [process](names-effects.md#process-calls), and
  [concurrency](names-effects.md#concurrency-calls).
  Prelude byte and protocol helper signatures, including HTTP/2 runtime
  diagnostic payload helpers, are listed in
  [Helper Signatures](names-effects.md#helper-signatures).
...
- Toolchain `std` package loading, implicit prelude imports, helper signatures,
  ...
  [names-effects.md](names-effects.md#prelude-helpers).
- Source-visible `StreamInput`, `StreamAdapterAction`, `DecodeStep<T>`,
  ...
  signatures:
  [standard byte ADTs](names-effects.md#standard-byte-adts) and
  [helper signatures](names-effects.md#helper-signatures).
- Descriptor-backed runtime symbols, including network stream write-side
  ...
  [names-effects.md](names-effects.md#compiler-known-descriptor-table).

## Fast Routes

- Checking the toolchain package, bootstrap exception, and compiler adapter
  boundary:
  [names-effects.md](names-effects.md#standard-package-boundary).
...
- Checking helper signatures before changing the prelude adapter:
  [names-effects.md](names-effects.md#helper-signatures).
...
- Do not open source-decision history before the implemented behavior in
  [names-effects.md](names-effects.md) answers the question.
...
owns standard-symbol lookup state. Shared standard-environment initialization
...
source lookup stay outside this registry gate. Embedded Veln prelude sources
