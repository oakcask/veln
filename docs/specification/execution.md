...
- Dispatch payload cases written with lowercase exact-width `uint...` and
  ...
  expose `()` as the payload value. The checked direct subbyte slice
  `uint1 reserves 0` consumes one payload byte, validates the high bit, emits
  the same storage byte during encode, and also exposes `()` as the payload
  value.
