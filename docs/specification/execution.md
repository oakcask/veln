...
- The socket stream adapter routing example composes the existing
  ...
  through `task::spawn_with10`, then to eleven ordinary source values
  through `task::spawn_with11`, and then to twelve ordinary source values
  through `task::spawn_with12`, and then to thirteen ordinary source values
  through `task::spawn_with13`, and then to fourteen ordinary source values
  through `task::spawn_with14`, and then to fifteen ordinary source values
  ...
  through `task::spawn_with22`, and then to twenty-three ordinary source
  values through `task::spawn_with23`, and then to twenty-four ordinary
  ...
  to twenty-eight ordinary source values through `task::spawn_with28`, and
  then to twenty-nine ordinary source values through `task::spawn_with29`,
  then to thirty ordinary source values through `task::spawn_with30`, and
  then to thirty-one ordinary source values through `task::spawn_with31`, and
  then to thirty-two ordinary source values through `task::spawn_with32`, and
  then to thirty-three ordinary source values through `task::spawn_with33`,
  and then to thirty-four ordinary source values through `task::spawn_with34`,
  and then to thirty-five ordinary source values through `task::spawn_with35`,
  and then to thirty-six ordinary source values through `task::spawn_with36`,
  and then translates ordered
...
- The channel-first stream routing examples route ordinary `StreamInput`
  ...
  twenty-nine-route, and receiver-list timeout typed channel
...
- Generated binary schema decode helpers support opt-in `Flag8`,
  ...
  `Flag48be`, `Flag48le`, `Flag64be`, and `Flag64le` fields as visible flag
  bitsets. They consume the same byte width, byte order, and truncation
  behavior as `UInt8`, `UInt16be`, `UInt16le`, `UInt24be`, `UInt24le`,
  `UInt32be`, `UInt32le`, `UInt48be`, `UInt48le`, `UInt64be`, and
  `UInt64le`, but the decoded record fields are source-visible
  `Flag8(bits)`, `Flag16be(bits)`, `Flag16le(bits)`, `Flag24be(bits)`,
  `Flag24le(bits)`, `Flag32be(bits)`, `Flag32le(bits)`,
  `Flag48be(bits)`, `Flag48le(bits)`, `Flag64be(bits)`, and
  `Flag64le(bits)` values rather than raw `Int` values.
  ...
  `Flag32be` and `Flag32le` bit indexes `0` through `31`, `Flag48be` and
  `Flag48le` bit indexes `0` through `47`, and `Flag64be` and `Flag64le` bit
  indexes `0` through `63`, returning `Result` failures for indexes outside
  each helper's range. Raw-bit helpers expose decoded `Flag8`, `Flag16be`,
  `Flag16le`, `Flag24be`, `Flag24le`, `Flag32be`, `Flag32le`, `Flag48be`,
  `Flag48le`, `Flag64be`, and `Flag64le` integer bits and construct flag
  values for encode only when the supplied integer fits the matching flag
  width.
  The checked examples are
...
- Eligible generated binary schema encode helpers named
  ...
  `Flag32be`, `Flag32le`, `Flag48be`, `Flag48le`, `Flag64be`, and
  `Flag64le` fields are source-visible `Flag8(bits)`, `Flag16be(bits)`,
  `Flag16le(bits)`, `Flag24be(bits)`, `Flag24le(bits)`, `Flag32be(bits)`,
  `Flag32le(bits)`, `Flag48be(bits)`, `Flag48le(bits)`, `Flag64be(bits)`, and
...
- The HTTP/2 protocol-core HPACK fixture boundary models HPACK as an imported
  ...
  `0x42 0x83 0x8c 0x72 0x7f`, and `0x12 0x83 0x8c 0x72 0x7f`, and
