...
- Generated binary schema decode helpers also support consecutive
  ...
  `examples/specification/run/binary-schema-packed-visible-six-byte-truncated-json/`,
...
- Generated binary schema decode helpers support bounded
  ...
  as `Int`, or `ByteView(left_length + right_length)` when both operands are
  earlier visible exact-width unsigned fields decoded as `Int`.
  ...
  a repeated `ByteView(length_field)` or
  `ByteView(left_length + right_length)` field decodes to
  ...
  `examples/specification/run/binary-schema-repeat-byteview-add-decode/`, and
...
- Eligible generated binary schema encode helpers named
  ...
  `ByteView(length_field)` or `ByteView(left_length + right_length)` fields are
  ...
  whose bounded byte count differs from the earlier length field or computed
  length expression, or a nested element
...
- The HPACK fixture boundary models HPACK as an imported ordinary source
  ...
  `content-range:`, `0x9f` `content-type:`, `0xa0` `cookie:`, `0xa1`
	  ...
	  unsupported-header-block diagnostic. The standalone
	  `hpack-fixture-codec-json` case directly decodes the same supported
	  one-byte indexed representation, and the checked
	  `hpack-static-index-unsupported-json` and
	  `hpack-static-index-unsupported-human` cases pin the focused
	  unsupported static-index projection. The fixture boundary also
...
- The same example keeps outbound DATA send-intent flow control separate from
  ...
  before output bytes or credit changes. A received
  `SETTINGS_INITIAL_WINDOW_SIZE` item applies the delta from the previous
  peer-advertised initial send window to the tracked open outbound stream send
  credit. Lowering that setting can make the stream send credit negative, and
  the same DATA payload is rejected through the existing stream send-window
  flow-control shape before output bytes or credit changes. Valid received
  stream-level `WINDOW_UPDATE` frames can then restore enough credit for the
  same DATA payload to be accepted, and increasing
  `SETTINGS_INITIAL_WINDOW_SIZE` raises existing open-stream send credit by
  the same delta. Valid received `WINDOW_UPDATE` frames can refill the
  separate outbound connection or stream send credit so the same DATA payload
  is accepted after matching peer credit is restored. Local outbound
  `WINDOW_UPDATE` send-intents still add receive credit only and do not refill
