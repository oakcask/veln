...
- Generated binary schema decode helpers also support `UInt16le`, `UInt24le`,
  `UInt31le`, `UInt32le`, `UInt48le`, and `UInt64le` as little-endian
  unsigned primitives.
  `UInt48be` is accepted as the matching big-endian six-byte primitive, and
...
- Generated binary schema decode helpers support opt-in `Flag8`,
  ...
  `UInt16be`, `UInt16le`, `UInt32be`, `UInt32le`, `UInt48be`, `UInt48le`,
  `UInt64be`, and `UInt64le` declarations continue to decode as ordinary
  `Int` fields. Pure prelude helpers inspect or set `Flag8` bit indexes `0`
...
- Generated binary schema decode helpers support bounded
  ...
  `UInt64le`, an eligible nested binary schema payload, or
...
- Eligible generated binary schema encode helpers named
  ...
  `UInt32le`, `UInt48le`, and `UInt64le` emit little-endian bytes and use the
  same representability boundaries as their matching unsigned widths.
  `UInt48be` emits big-endian six-byte values, and `UInt64be` emits
  big-endian eight-byte values. Standalone visible `UInt1` through `UInt7`
...
- The HTTP/2 protocol-core HPACK fixture boundary models HPACK as an imported
  ...
  `content-range:`, `0x9f` `content-type:`, `0xa0` `cookie:`, `0xa1`
	  `date:`, `0xa2` `etag:`, `0xa3` `expect:`, `0xa4` `from:`,
	  `0xa5` `host:`, `0xa6` `if-match:`, `0xa7` `if-modified-since:`,
	  `0xa8` `if-none-match:`, `0xa9` `if-range:`, `0xaa`
  ...
  update fixtures `0x3e`, `0x3f`, `0x3f` followed by one non-continuing
  HPACK integer continuation byte, and the explicit multi-continuation
  fixture `0x3f 0x81 0x01`. Those fixtures return next immutable
  fixture states with table sizes `30`, `31`, the saturated prefix value
  plus the one-byte continuation value, or `160` from either a completed
  HEADERS block or a final CONTINUATION block before a later header block is
  decoded. When
  ...
  input, including general HPACK integer encodings outside those explicit
  table-size update bytes, malformed literal-without-indexing, and unsupported
  Huffman variants, projects through
