...
- [open-questions.md](agent-language-spec-wall/open-questions.md)
...
- Add each result as one file under
  `agent-language-spec-wall/` named
...
- [Block Structure](agent-language-spec-wall/result-block-structure.md):
...
- [Compact Function Form](agent-language-spec-wall/result-compact-function-form.md):
...
- [Method Call Boundary](agent-language-spec-wall/result-method-call-boundary.md):
...
- [JSON Diagnostic Schema Stability](agent-language-spec-wall/result-json-diagnostic-schema-stability.md):
  use a stable top-level diagnostic envelope while allowing prototype
  kind-specific `details` payloads to change.
- [Public Function Type Boundaries](agent-language-spec-wall/result-public-function-type-boundaries.md):
...
- [Minimum Type System for Holes](agent-language-spec-wall/result-minimum-type-system-for-holes.md):
  ...
  diagnostics.
- [User-Defined ADTs in the First Slice](agent-language-spec-wall/result-user-defined-adts-first-slice.md):
...
- [Primary Check Command](agent-language-spec-wall/result-primary-check-command.md):
  make `veln check` the primary read-only command for parse, type, contract,
  effect, lint, doc drift, and hole diagnostics.
- [First Implementation Commands](agent-language-spec-wall/result-first-implementation-commands.md):
  require `check`, `fmt`, `run`, and `test` in the first implementation, while
  deferring `doc`, `graph`, `explain`, and `repair`.
- [Test JSON Shape](agent-language-spec-wall/result-test-json-shape.md):
...
- [Affected Test Selection](agent-language-spec-wall/result-affected-test-selection.md):
...
- [Minimal Project and Test Discovery](agent-language-spec-wall/result-minimal-project-test-discovery.md):
...
- [Hole Runtime Boundary](agent-language-spec-wall/result-hole-runtime-boundary.md):
  allow files with holes to be checked, but block `run` when a hole may be
  reachable from the selected entry point.
- [Hole Diagnostic JSON Shape](agent-language-spec-wall/result-hole-diagnostic-json-shape.md):
  report unfilled holes as stable-envelope `kind: "hole"` diagnostics with a
  prototype repair-context `details` payload.
- [Named Hole Syntax](agent-language-spec-wall/result-named-hole-syntax.md):
...
- [Error Type Inference](agent-language-spec-wall/result-error-type-inference.md):
...
- [First-Slice Value Mutability](agent-language-spec-wall/result-first-slice-value-mutability.md):
...
- [First Implementation Runtime Targets](agent-language-spec-wall/result-first-implementation-runtime-targets.md):
...
- [First Implementation Architecture](agent-language-spec-wall/result-first-implementation-architecture.md):
...
- [Channel-First Concurrency Runtime](agent-language-spec-wall/result-channel-first-concurrency-runtime.md):
...
- [Runtime Value Freeze Boundary](agent-language-spec-wall/result-runtime-value-freeze-boundary.md):
...
- [Pipeline Style](agent-language-spec-wall/result-pipeline-style.md):
...
- [Contract Blame Boundary](agent-language-spec-wall/result-contract-blame-boundary.md):
...
- [Contract Expression Language](agent-language-spec-wall/result-contract-expression-language.md):
...
- [Contract Static Runtime Boundary](agent-language-spec-wall/result-contract-static-runtime-boundary.md):
...
- [Effect Declaration Boundary](agent-language-spec-wall/result-effect-declaration-boundary.md):
...
- [Effect Access Modes](agent-language-spec-wall/result-effect-access-modes.md):
...
- [First-Slice Observable I/O](agent-language-spec-wall/result-first-slice-observable-io.md):
...
- [Stdio API and Output Events](agent-language-spec-wall/result-stdio-api-and-output-events.md):
  provide four string-only stdio output functions and capture deterministic
  source-linked stdout/stderr events for tests.
- [Transitive Effect Diagnostics](agent-language-spec-wall/result-transitive-effect-diagnostics.md):
  report missing public effects by coarse label with bounded provenance slices
  instead of full transitive call graphs.
- [Postcondition Result Binding](agent-language-spec-wall/result-postcondition-result-binding.md):
  use an explicit contract-facing result binding for postconditions instead of
  a magic bare `result` identifier.
- [Hole Satisfy Constraint Grammar](agent-language-spec-wall/result-hole-satisfy-constraint-grammar.md):
...
- [Hole Satisfy Source Syntax](agent-language-spec-wall/result-hole-satisfy-source-syntax.md):
...
- [Satisfy Unknown Severity](agent-language-spec-wall/result-satisfy-unknown-severity.md):
  ...
  workflow.
- [Module Metadata Location](agent-language-spec-wall/result-module-metadata-location.md):
...
- [First-Slice Module Fields](agent-language-spec-wall/result-first-slice-module-fields.md):
  ...
  optional.
- [ADR-Lite Decision Location](agent-language-spec-wall/result-adr-lite-decision-location.md):
...
- [Doctest Result Propagation](agent-language-spec-wall/result-doctest-result-propagation.md):
...
- [Doctest Error Type Fence Syntax](agent-language-spec-wall/result-doctest-error-type-fence-syntax.md):
...
- [Doctest Expected Output Syntax](agent-language-spec-wall/result-doctest-expected-output-syntax.md):
...
- [AST Phase Boundary](agent-language-spec-wall/result-ast-phase-boundary.md):
  ...
  diagnostics.
- [AST Implementation Representation](agent-language-spec-wall/result-ast-implementation-representation.md):
...
- [Check JSON Details Fields](agent-language-spec-wall/result-check-json-details-fields.md):
...
- [Safe Repair Candidate Boundary](agent-language-spec-wall/result-safe-repair-candidate-boundary.md):
...
- [Scoping and Name Resolution](agent-language-spec-wall/result-scoping-and-name-resolution.md):
...
- [Prelude Complexity Guarantees](agent-language-spec-wall/result-prelude-complexity-guarantees.md):
  keep first-slice prelude helper complexity non-normative until concrete
  persistent container representations are chosen.
- [Comparison Example Task](agent-language-spec-wall/result-comparison-example-task.md):
