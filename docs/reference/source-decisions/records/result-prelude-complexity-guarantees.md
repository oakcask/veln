...
- What complexity guarantees, if any, should first-slice prelude helpers make
  before Veln has chosen concrete persistent list and dictionary
  representations?
...
`list_try_map`, and deterministic diagnostics. Complexity claims remain
implementation notes until the language chooses and documents concrete
container representations.

In particular, the language spec should not say that `list_push`,
`list_concat`, `dict_insert`, or `dict_remove` are `O(1)`, `O(log n)`, or
...
`list_try_map`: traverse in order while sequencing a surrounding effect. Rust's
...
`list_try_map` preserves element order, and that the first `Err` is returned
...
- `list_map`, `list_filter`, `list_fold`, and `list_try_map` evaluate list
  elements in source order.
- `list_try_map` stops at the first `Err`, returns that error unchanged, and
  does not evaluate later elements.
- `list_push`, `list_concat`, `dict_insert`, and `dict_remove` return new
