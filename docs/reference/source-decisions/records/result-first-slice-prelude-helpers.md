...
generics, traits, or type classes. A name such as `vec_try_map` is therefore
clearer than a free `try_map` whose dispatch rules would be invisible.

Required first-slice helpers:

- Vecs: `vec_len`, `vec_is_empty`, `vec_push`, `vec_concat`,
...
`vec_try_map(items, f)` has this semantic shape:

```text
vec_try_map(Vec<A>, fn(A) -> Result<B, E>) -> Result<Vec<B>, E>
```

It evaluates items in source order, stops at the first `Err`, returns that
error unchanged, and otherwise returns `Ok` with the mapped vec in the same
order. It is the standard first-slice way to apply `?`-producing work across a
vec. `?` still propagates only inside the result-returning function that calls
`vec_try_map`; it does not gain special pipeline behavior.
...
`vec_push(existing, value)` are acceptable even if an implementation later
...
`vec_try_map`, not a generic `traverse`.
...
behavior direct and diagnostic-friendly: when an agent writes `vec_map` with a
function returning `Result`, `veln check` can suggest `vec_try_map` and show
...
- `vec_push` returns a new vec with the value appended at the end.
- `vec_concat` returns a new vec containing the left vec followed by the
  right vec.
- `dict_insert` and `dict_remove` return new dictionaries; they do not mutate
  the input dictionary.
- `dict_get` returns `Option<V>`.
- `vec_map` accepts a pure value-returning callback. If the callback returns
  `Result`, diagnostics should suggest `vec_try_map`.
- `vec_try_map` is the only first-slice fallible traversal helper required by
  examples and tests.
...
Names for indexed operations such as `vec_at`, `vec_take`, `vec_drop`,
...
`vec_try_map` over a callback returning `Result`.
