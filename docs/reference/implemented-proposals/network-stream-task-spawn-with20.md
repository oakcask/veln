...
This record preserves the completed twenty-argument stream-handler task
...
The completed slice adds `task::spawn_with20` as the twenty-argument
...
`task::spawn_with16`, `task::spawn_with17`, `task::spawn_with18`, and
`task::spawn_with19`. It starts a twenty-argument callable under the
...
context, routing metadata, and sixteen additional ordinary metadata values as
...
- Auditing why twenty-argument stream-handler task spawning is no longer
