...
This record preserves the completed twenty-one-argument stream-handler task
...
The completed slice adds `task::spawn_with21` as the twenty-one-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
twenty-one-argument callable under the existing `concurrency` effect, accepts
...
context, routing metadata, and seventeen additional ordinary metadata values
...
- Auditing why twenty-one-argument stream-handler task spawning is no longer
