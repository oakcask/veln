...
This record preserves the completed visible-only packed sub-byte eight-byte
...
eight-byte big-endian storage unit. The fields are packed in declaration
...
Truncation reports `schema.truncated_field` at the packed visible field path
corresponding to the missing input position, with the existing structured
byte diagnostic counts. Encode range failures keep the existing
`schema.encode_value_unrepresentable` shape at the schema-local visible field
path.
...
- `../../../examples/specification/run/binary-schema-packed-visible-eight-byte-truncated-json/`
  checks JSON truncation details for a short eight-byte packed visible group.
