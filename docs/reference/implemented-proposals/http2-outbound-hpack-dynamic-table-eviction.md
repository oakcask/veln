...
is exactly `42`. After a table-size update to `30`, the fixture emits the
literal-with-indexing bytes for `:method: PUT`, but the returned state cannot
retain that entry; encoding the same header list again emits the literal bytes
again instead of a dynamic indexed byte. After a table-size update to `42`,
the same literal-with-indexing encode retains the entry, and the following
matching encode emits `0xbe`.

The HTTP/2 outbound HEADERS path consumes those returned fixture states before
frame splitting. The aggregate protocol-core case therefore observes both the
literal-again behavior at table size `30` and the dynamic-index reuse at table
size `42` as encoded HEADERS output chunks.
...
- `../../../examples/specification/run/hpack-fixture-codec-boundary/` checks
  direct fixture encoder transitions for table size `30` and table size `42`,
  including the second encode that proves whether `:method: PUT` was retained.
- `../../../examples/specification/run/http2-protocol-core/` routes the same
  state transitions through outbound HEADERS and checks the emitted frame
  chunks for the literal and dynamic-indexed outcomes.
