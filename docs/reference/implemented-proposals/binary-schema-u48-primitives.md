# Binary Schema UInt48 Primitives

Status: implemented

This record preserves the completed `UInt48be` and `UInt48le` exact-width
...
`UInt48be` and `UInt48le` are accepted only as `format binary` schema field
primitive names. Generated decode helpers consume exactly six bytes in the
declared byte order and expose visible fields as ordinary `Int` values.
Generated encode helpers accept ordinary `Int` fields, emit exactly six bytes
...
- `../../../examples/specification/check/schema-exact-width-primitive-diagnostics/`
  checks that `UInt48be` and `UInt48le` remain schema-local names.
