...
This record preserves the completed five-argument stream-handler task slice
...
The completed slice adds `task::spawn_with5` as the five-argument sibling of
`task::spawn_with`, `task::spawn_with2`, `task::spawn_with3`, and
`task::spawn_with4`. It starts a five-argument callable under the existing
...
context, routing metadata, and one additional ordinary metadata value as
...
- Auditing why five-argument stream-handler task spawning is no longer active
  proposal work.
