...
This record preserves the completed thirty-seven-argument stream-handler task
...
The completed slice adds `task::spawn_with37` as the thirty-seven-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
thirty-seven-argument callable under the existing `concurrency` effect,
accepts the same optional return-type argument shape, and returns `Task<T>`.

Executable stream adapter coverage passes ordinary event, state, adapter
context, routing metadata, and thirty-three additional ordinary metadata
values as separate task arguments. The spawned handler receives no `NetStream`
or other transport handle, and the slice adds no `net`, `time`, routing, or
...
- Auditing why thirty-seven-argument stream-handler task spawning is no
