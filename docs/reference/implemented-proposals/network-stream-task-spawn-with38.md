...
This record preserves the completed thirty-eight-argument stream-handler task
...
The completed slice adds `task::spawn_with38` as the thirty-eight-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
thirty-eight-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and thirty-four additional ordinary metadata values
...
- Auditing why thirty-eight-argument stream-handler task spawning is no
