...
This record preserves the completed twenty-four-argument stream-handler task
...
The completed slice adds `task::spawn_with24` as the twenty-four-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
twenty-four-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and twenty additional ordinary metadata values as
...
- Auditing why twenty-four-argument stream-handler task spawning is no longer
