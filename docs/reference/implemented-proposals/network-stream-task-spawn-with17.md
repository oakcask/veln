...
This record preserves the completed seventeen-argument stream-handler task
...
The completed slice adds `task::spawn_with17` as the seventeen-argument
...
`task::spawn_with16`. It starts a seventeen-argument callable under the
...
context, routing metadata, and thirteen additional ordinary metadata values as
...
- Auditing why seventeen-argument stream-handler task spawning is no longer
