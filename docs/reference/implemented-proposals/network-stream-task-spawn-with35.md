...
This record preserves the completed thirty-five-argument stream-handler task
...
The completed slice adds `task::spawn_with35` as the thirty-five-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
thirty-five-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and thirty-one additional ordinary metadata
...
- Auditing why thirty-five-argument stream-handler task spawning is no
