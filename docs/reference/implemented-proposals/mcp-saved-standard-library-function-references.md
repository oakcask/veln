...
- The `references-standard-library-function` executable MCP specification case
  checks accepted bare implicit prelude calls, bare function-value
  occurrences, qualified calls, qualified function-value occurrences,
  workspace `file:` result locations,
...
- `veln-language-service` tests check explicit standard-library module
  function references, implicit prelude function references, bare and
  qualified function-value references, package-origin identity, lexical
  shadowing, collision exclusion, private and non-exported visibility, and
