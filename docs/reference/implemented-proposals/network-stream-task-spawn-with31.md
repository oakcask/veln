...
This record preserves the completed thirty-one-argument stream-handler task
...
The completed slice adds `task::spawn_with31` as the thirty-one-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
thirty-one-argument callable under the existing `concurrency` effect, accepts
...
context, routing metadata, and twenty-seven additional ordinary metadata
...
- Auditing why thirty-one-argument stream-handler task spawning is no longer
