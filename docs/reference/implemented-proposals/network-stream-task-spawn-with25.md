...
This record preserves the completed twenty-five-argument stream-handler task
...
The completed slice adds `task::spawn_with25` as the twenty-five-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
twenty-five-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and twenty-one additional ordinary metadata values as
...
- Auditing why twenty-five-argument stream-handler task spawning is no longer
