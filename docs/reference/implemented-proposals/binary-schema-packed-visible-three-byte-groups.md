...
This record preserves the completed visible-only packed sub-byte three-byte
...
three-byte big-endian storage unit. The fields are packed in declaration
...
group with a three-byte expected count. Encode range failures keep the
...
- `../../../examples/specification/run/binary-schema-packed-visible-three-byte-truncated-json/`
  checks JSON truncation details for a short three-byte packed visible group.
