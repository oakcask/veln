...
This record preserves the completed thirty-argument stream-handler task
...
The completed slice adds `task::spawn_with30` as the thirty-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
thirty-argument callable under the existing `concurrency` effect, accepts
...
context, routing metadata, and twenty-six additional ordinary metadata values
...
- Auditing why thirty-argument stream-handler task spawning is no longer
