...
This record preserves the completed sixteen-argument stream-handler task
...
The completed slice adds `task::spawn_with16` as the sixteen-argument sibling
of `task::spawn_with`, `task::spawn_with2`, `task::spawn_with3`,
...
`task::spawn_with13`, `task::spawn_with14`, and `task::spawn_with15`. It
starts a sixteen-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and twelve additional ordinary metadata values as
...
- Auditing why sixteen-argument stream-handler task spawning is no longer
