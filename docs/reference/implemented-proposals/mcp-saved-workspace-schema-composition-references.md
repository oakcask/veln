...
| Written import paths and implicit leaf aliases share semantic composition resolution, while bare imports remain rejected. | `schema_composition_resolves_workspace_import_leaf_aliases_before_collision_checks`, `workspace_schema_references_cover_direct_and_repeated_composition_targets`, and the MCP `references-workspace-schema-composition` case |
| Visibility, shadowing, and exact companion access preserve identity boundaries. | `workspace_schema_references_include_exact_companion_private_qualified_uses` and `references_keep_workspace_schema_identity_visibility_and_companion_boundaries` |
