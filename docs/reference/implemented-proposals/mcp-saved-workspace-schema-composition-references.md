...
This earlier workspace-composition slice did not add schema-alias identity or
alias-chain references, package-schema composition references, declaration
inclusion, package-source locations, pagination, recovery or casing-neutral
selection, transitive-dependency references, rename behavior, or MCP schema
expansion. A later slice added `decode` and `encode` operation references for
eligible public schemas from retained direct dependencies; see
...
| Alias-chain traversal and module-qualifier selections remain unsupported; package-schema composition selections, recovery records, and casing-neutral selections retain the shared successful-empty boundary. Direct-dependency schema operation leaves use the later operation-reference boundary. | `workspace_schema_references_keep_schema_specific_unsupported_selections_empty`, `package_composition_does_not_bind_same_named_workspace_schema`, `dependency_composition_with_matching_source_identity_stays_isolated`, `references_reject_recovery_package_and_unsupported_symbols`, and [MCP Saved Direct-Dependency Schema Operation References](mcp-saved-dependency-schema-operation-references.md) |
