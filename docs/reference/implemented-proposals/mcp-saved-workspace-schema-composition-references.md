...
| Alias-chain traversal and module-qualifier selections remain unsupported; package schemas, recovery records, and casing-neutral selections retain the shared successful-empty boundary. | `workspace_schema_references_keep_schema_specific_unsupported_selections_empty`, `package_composition_does_not_bind_same_named_workspace_schema`, `dependency_composition_with_matching_source_identity_stays_isolated`, and `references_reject_recovery_package_and_unsupported_symbols` |
