...
This record documents the removal of schema-level `map to` clauses from the
language surface.
...
The following source syntax is no longer accepted:
...
Schema declarations contain only representation fields, format selection,
field-local predicates, and schema-level validation. They do not name an
internal value type and do not contain mapping assignment lines.
