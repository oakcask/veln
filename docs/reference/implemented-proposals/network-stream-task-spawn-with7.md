...
This record preserves the completed seven-argument stream-handler task slice
...
The completed slice adds `task::spawn_with7` as the seven-argument sibling of
`task::spawn_with`, `task::spawn_with2`, `task::spawn_with3`,
`task::spawn_with4`, `task::spawn_with5`, and `task::spawn_with6`. It starts a
seven-argument callable under the existing `concurrency` effect, accepts the
...
context, routing metadata, and three additional ordinary metadata values as
...
- Auditing why seven-argument stream-handler task spawning is no longer active
  proposal work.
