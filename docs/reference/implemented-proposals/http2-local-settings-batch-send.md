...
The constrained local item ranges remain:
`SETTINGS_ENABLE_PUSH` accepts `0..1`,
`SETTINGS_INITIAL_WINDOW_SIZE` accepts `0..2147483647`, and
`SETTINGS_MAX_FRAME_SIZE` accepts `16384..16777215`. An out-of-range item
inside a larger batch is rejected before output bytes are emitted and uses
...
- `../../../examples/specification/run/http2-protocol-core/` checks a
  ...
  outstanding, and the no-output range-diagnostic path for an invalid item
