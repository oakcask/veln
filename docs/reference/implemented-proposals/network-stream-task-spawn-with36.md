...
This record preserves the completed thirty-six-argument stream-handler task
...
The completed slice adds `task::spawn_with36` as the thirty-six-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
thirty-six-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and thirty-two additional ordinary metadata
...
- Auditing why thirty-six-argument stream-handler task spawning is no
