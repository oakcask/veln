...
This record preserves the completed eight-argument stream-handler task slice
...
The completed slice adds `task::spawn_with8` as the eight-argument sibling of
`task::spawn_with`, `task::spawn_with2`, `task::spawn_with3`,
`task::spawn_with4`, `task::spawn_with5`, `task::spawn_with6`, and
`task::spawn_with7`. It starts an eight-argument callable under the existing
`concurrency` effect, accepts the same optional return-type type argument
shape, and returns `Task<T>`.

Executable stream adapter coverage passes ordinary event, state, adapter
context, routing metadata, and four additional ordinary metadata values as
...
- Auditing why eight-argument stream-handler task spawning is no longer active
  proposal work.
