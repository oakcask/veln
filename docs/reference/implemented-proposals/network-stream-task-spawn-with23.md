...
This record preserves the completed twenty-three-argument stream-handler task
...
The completed slice adds `task::spawn_with23` as the twenty-three-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
twenty-three-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and nineteen additional ordinary metadata values as
...
- Auditing why twenty-three-argument stream-handler task spawning is no longer
