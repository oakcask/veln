...
This record preserves the completed visible-only packed sub-byte four-byte
...
four-byte big-endian storage unit. The fields are packed in declaration order
...
group with a four-byte expected count. Encode range failures keep the
...
- `../../../examples/specification/run/binary-schema-packed-visible-four-byte-truncated-json/`
  checks JSON truncation details for a short four-byte packed visible group.
