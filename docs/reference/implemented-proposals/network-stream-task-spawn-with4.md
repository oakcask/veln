...
This record preserves the completed four-argument stream-handler task slice
...
The completed slice adds `task::spawn_with4` as the four-argument sibling of
`task::spawn_with`, `task::spawn_with2`, and `task::spawn_with3`. It starts a
four-argument callable under the existing `concurrency` effect, accepts the
...
context, and routing metadata values as separate task arguments. The spawned
handler receives no `NetStream` or other transport handle, and the slice adds
no `net`, `time`, routing, or task-specific effect label beyond
`concurrency`.
...
- Auditing why four-argument stream-handler task spawning is no longer active
  proposal work.
