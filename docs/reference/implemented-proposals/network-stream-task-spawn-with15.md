...
This record preserves the completed fifteen-argument stream-handler task
...
The completed slice adds `task::spawn_with15` as the fifteen-argument sibling
...
`task::spawn_with13`, and `task::spawn_with14`. It starts a
fifteen-argument callable under the existing `concurrency` effect, accepts the
same optional return-type argument shape, and returns `Task<T>`.

Executable stream adapter coverage passes ordinary event, state, adapter
context, routing metadata, and eleven additional ordinary metadata values as
...
- Auditing why fifteen-argument stream-handler task spawning is no longer
