...
update-when: The MCP JSONL equality, string-containment, length, missing-path, workspace URI, definition-workspace executable MCP case, shared navigation capture evidence, or saved workspace references slice boundary is superseded.
---
...
- The toolchain harness accepts `[[mcp_assert]]` for newline-delimited MCP
  ...
  RFC 6901 JSON Pointer paths with complete JSON equality, string
  containment, array length, missing-path assertions, and canonical
  workspace-file URI assertions.
