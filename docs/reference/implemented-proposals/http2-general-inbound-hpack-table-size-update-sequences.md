...
and absolute HPACK byte offsets. A failed block exposes neither a partial
header list nor a partially updated state, so callers can reuse the original
input state.
...
- Completed HEADERS, `PUSH_PROMISE`, and final CONTINUATION cases cover the
  shared success path.
- Late excessive and malformed updates cover focused failure routing, while
  retry and state-equality checks cover input-state reuse and atomicity.
