...
This record preserves the completed twenty-eight-argument stream-handler task
...
The completed slice adds `task::spawn_with28` as the twenty-eight-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
twenty-eight-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and twenty-four additional ordinary metadata values
...
- Auditing why twenty-eight-argument stream-handler task spawning is no longer
