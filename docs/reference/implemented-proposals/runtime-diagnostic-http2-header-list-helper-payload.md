...
`http2_peer_limit_header_list_size_exceeded(...)` now returns
`Result<(), RuntimeDiagnostic>`. On failure it returns
`Err(RuntimeDiagnostic(...))` with
`RuntimeHttp2PeerLimitHeaderListSizeDiagnostic(...)` carrying the byte offset,
observed header-list size, allowed header-list size, frame kind, stream id,
receive-limit provenance, rule provenance, and bounded header-block preview.
...
- `../../../examples/specification/run/runtime-diagnostic-http2-header-list-helper-json/`
  ...
  `http2_peer_limit_header_list_size_exceeded(...)` returns a rendered
