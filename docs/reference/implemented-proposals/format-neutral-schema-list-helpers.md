...
`byte_decode_<schema>` helpers when a top-level field is `List<Int>` or
`List<String>`. The helper remains a validation/pass-through boundary over the
schema-local visible record shape and returns `Result<TRecord, String>`.
...
- `../../../examples/specification/run/format-neutral-schema-decode/` checks
  successful top-level `List<Int>` and `List<String>` fields beside the
  existing scalar, nested record-shaped, and supported `Option` fields.
- `../../../examples/specification/check/format-neutral-schema-decode-helper-diagnostics/`
  keeps diagnostics for unsupported `Option<List<Int>>`, unsupported list
  payloads, nested record-contained lists, and unrelated container shapes.
