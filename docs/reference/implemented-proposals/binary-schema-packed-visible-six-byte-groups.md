...
six-byte big-endian storage unit. The fields are packed in declaration order
...
group with a six-byte expected count. Encode range failures keep the existing
...
- `../../../examples/specification/run/binary-schema-packed-visible-six-byte-truncated-json/`
  checks JSON truncation details for a short six-byte packed visible group.
...
seven-byte and eight-byte visible-only packed groups plus reserved-bit
layouts, dispatch forms, primitive shapes, and mapping behavior outside the
