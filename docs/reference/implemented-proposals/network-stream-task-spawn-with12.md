...
This record preserves the completed twelve-argument stream-handler task slice
...
The completed slice adds `task::spawn_with12` as the twelve-argument sibling of
...
`task::spawn_with10`, and `task::spawn_with11`. It starts a twelve-argument
callable under the existing `concurrency` effect, accepts the same optional
return-type argument shape, and returns `Task<T>`.

Executable stream adapter coverage passes ordinary event, state, adapter
context, routing metadata, and eight additional ordinary metadata values as
...
- Auditing why twelve-argument stream-handler task spawning is no longer
