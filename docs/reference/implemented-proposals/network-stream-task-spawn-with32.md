...
This record preserves the completed thirty-two-argument stream-handler task
...
The completed slice adds `task::spawn_with32` as the thirty-two-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
thirty-two-argument callable under the existing `concurrency` effect, accepts
...
context, routing metadata, and twenty-eight additional ordinary metadata
...
- Auditing why thirty-two-argument stream-handler task spawning is no longer
