...
extension dispatch payload cases written as `uint2 reserves 0`.

The payload is representation-only. Decode validates the high two bits of the
payload storage byte, reports `schema.reserved_bits_mismatch` at the payload
byte offset when the bits are not zero, and exposes `()` as the payload value.
Encode writes the fixed zero bits as a one-byte payload. Extension dispatch
