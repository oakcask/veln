...
- The `references-workspace` executable MCP specification case checks tool
  ...
  unsupported constructor success, invalid positions, and schema-invalid
  coordinates.
- `veln-mcp` server tests check selected-project inference, single-file
  ...
  unsupported-symbol success, path and coordinate failures, result schema
  acceptance, and stable-capture failure without partial reference locations.
