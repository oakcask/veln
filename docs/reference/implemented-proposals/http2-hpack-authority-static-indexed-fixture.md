...
fixture. Unsupported bytes and unsupported multi-byte indexed forms still
project through
...
- `../../../examples/specification/run/hpack-fixture-codec-boundary/` checks
  the focused `authority` decode, the corrected `expires` entry, and the
  shifted later static indexed entries through `www-authenticate`.
- `../../../examples/specification/run/http2-protocol-core/` checks the
  ...
  `hpack-static-indexed-expires`, `hpack-static-indexed-from`, and the later
  entries through `hpack-static-indexed-www-authenticate`; the checked output
  emits `a4` for `expires`, `a5` for `from`, and `bd` for
  `www-authenticate`.
