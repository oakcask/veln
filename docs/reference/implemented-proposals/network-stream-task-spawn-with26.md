...
This record preserves the completed twenty-six-argument stream-handler task
...
The completed slice adds `task::spawn_with26` as the twenty-six-argument
sibling of the earlier `task::spawn_withN` helpers. It starts a
twenty-six-argument callable under the existing `concurrency` effect,
...
context, routing metadata, and twenty-two additional ordinary metadata values as
...
- Auditing why twenty-six-argument stream-handler task spawning is no longer
